...
- [x] WorkerProfile / capability pack 与主 Agent Profile + Context Continuity 主链均已交付；bootstrap/operator UX 仍可继续增强
...
- [x] Feature 033：Agent Profile + Bootstrap + Context Continuity（主 Agent runtime 已真实接入 profile / context frame / recent context / memory retrieval；剩余 bootstrap/operator UX 不再阻塞 gate）
...
- 必须明确 `project -> session -> work` 与 `project -> automation -> work` 两条继承链：project 提供默认 bindings，session/automation 选择 agent profile，work 继承 effective config 并允许显式覆盖少数字段
...
| `Session` | 严格隶属于一个 project | chat lifecycle、history、queue、focus、effective config snapshot | project + selected agent profile | 交互态对象，不直接承载长期 secret 实值 |
| `Automation` | 严格隶属于一个 project | schedule、trigger、target、run history、effective config snapshot | project + selected agent profile | 可创建 session 或直接派生 work |
...
- 033 已完成主 Agent 的 context continuity 主链，M3 现在可以按“具备长期上下文连续性”的 user-ready 版本对外描述
- 031 的 release gate artifacts 已同步关闭 `GATE-M3-CONTEXT-CONTINUITY`
