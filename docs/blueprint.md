...
正确做法（参考 OpenClaw `security=full/allowlist/deny` 维度）：
- 路径越界不应在工具层硬拒绝，应走 Policy Engine → `PolicyAction.ASK`
- 用户可通过 `ApprovalDecision.ALLOW_ONCE / ALLOW_ALWAYS` 授权
- `terminal.exec` 的路径访问应与 `filesystem.*` 策略对齐
