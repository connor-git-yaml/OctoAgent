...
- NG4：不在 v0.x 阶段把所有子流程都 Pipeline 化
  - 主 Agent、Workers、Subagent 都永远 Free Loop；Skill Pipeline（Graph）仅用于有副作用/需要 checkpoint 的子流程，按需引入
...
##### 文件系统路径访问策略（待对齐 Policy Engine）

当前 `filesystem.*` 工具硬编码限制 workspace 内路径，workspace 外路径直接 `raise RuntimeError`，**绕过了 Policy Engine 的审批链路**。而 `terminal.exec` 不受路径限制，形成安全策略不一致。
...
**设计原则**：所有工具的路径/权限检查统一由 Policy Engine 管辖，工具层不得硬编码绕过：
- `PolicyAction.ALLOW` → workspace 外路径直接放行（对齐 `security=full`）
- `PolicyAction.ASK` → workspace 外路径走审批（用户可 `ALLOW_ONCE / ALLOW_ALWAYS / DENY`）
- `PolicyAction.DENY` → 拒绝
- `terminal.exec` 也应纳入同样的路径治理
