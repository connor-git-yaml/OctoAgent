...
- M11 启动日期：2026-07-19；2026-07-20 完成 F148，二次架构复审后顺序调整为 F151 → F150 → F149（F150 live spike 可提前）；2026-07-22 F151 已收敛 Gateway 唯一 module entry、静态 exit 64/78 与 descriptor 只读边界，后续任务继续推进
- 目标读者：
  - 你（Owner / PM / 架构师 / 最终用户）
...
| [codebase-architecture/provider-direct-routing.md](codebase-architecture/provider-direct-routing.md) | F080/F081 ProviderRouter 直连路由 |
...
- 执行：由单一 Gateway application host 内的 TaskRunner、Inline/Graph backend 与工具门禁承载，可回放，产物（Artifacts）可追溯；当前没有已实现的 Docker / SSH / 远程隔离 backend
- 记忆：有治理（SoR/Fragments 双线 + 版本化 + 冲突仲裁 + Vault 分区）
- 模型：统一经 ProviderRouter 直连模型提供商；业务代码只使用 alias 与策略路由
...
4) **Side-effect Must be Two-Phase（副作用必须二段式）**
   - 不可逆操作必须拆成：`Plan`（无副作用）→ Gate（规则/人审）→ `Execute`。
...
4) **记忆写入必须治理**
   - 高风险记忆写入（跨 project、覆盖已有事实）需要治理流程（WriteProposal → 验证 → 提交）。
   - 低风险场景（单 project 内新增事实）可走 fast_commit 快速路径。
...
- G6：统一模型出口与成本治理：ProviderRouter + alias 路由 + fallback + 统计
...
```
Channels (Telegram/Web) → Gateway 单一运行时 → ProviderRouter → Model Providers
```
...
- **ProviderRouter**：统一模型路由（alias + fallback + 成本统计），直接调用 Provider
...
- Python 3.12+（主工程）
- uv（依赖与环境管理）
- 进程内 Inline/Graph runtime（当前无 Docker/SSH 隔离 backend）
