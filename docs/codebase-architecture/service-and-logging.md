...
| 模块 | 职责 |
|------|------|
| `packages/provider/src/octoagent/provider/dx/service_manager.py` | `ServiceBackend` 抽象 + `LaunchdBackend`/`SystemdUserBackend` + `ServiceManager` 编排（install 三态幂等 / uninstall 尽力清理+残留复查 / status 三态并行探测）+ `detect_init_system` + `resolve_instance_root` + 稳定路径校验 |
| `packages/provider/src/octoagent/provider/dx/service_commands.py` | `octo service {install,uninstall,status}` + `octo logs`（tail/-f/-n/--level + 启动期崩溃回退 err.log；全部输出过脱敏）|
| `packages/provider/src/octoagent/provider/dx/sleep_probe.py` | 睡眠风险只读探测（pmset -g / 电池检测；**绝不写系统设置**）|
| `packages/provider/src/octoagent/provider/dx/doctor.py`（扩展）| `check_service_status`（三态+linger+ready 健康映射）+ `check_sleep_settings`（WARN+平台化 fix_hint）|
