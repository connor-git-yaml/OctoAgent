...
> **SPA catch-all 与 ingress 顺序不变量**（F105 v0.2 潜伏 production bug 修复，2026-07-12）：inbound router 在 **lifespan bootstrap 段**挂载，而前端 SPA catch-all（`Mount("/")`，全前缀匹配一切）必须挂成**最后一条路由**。Starlette 按注册序匹配——若 SPA 排在 lifespan 期挂载的 inbound 路由**之前**，会把它们全部遮蔽：`POST /api/telegram/webhook` 落到 SPA 的 `StaticFiles`（只收 GET/HEAD）→ **405**（只要 `frontend/dist` 存在即触发，即任何 build 过 Web UI 的部署/开发机）。故 SPA 挂载**不在 `create_app` 构造期**做，而由 `main.py:_mount_frontend_spa` 在 `lifespan` / `_make_harness_lifespan` 的 `harness.commit_to_app` **之后**调用（恒为最后一条路由）。回归护栏：`test_f105v02_ingress.py::test_frontend_spa_mount_does_not_shadow_webhook_when_dist_exists`。
