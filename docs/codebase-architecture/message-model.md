...
| `requested_worker_profile_id` / `requested_worker_profile_version` | str / int | 申请 Root Agent profile（`agent_profiles` kind=worker）+ revision（F117 D2 后 WorkerProfile 类已并入 AgentProfile；字段名待 W3 改 `requested_agent_profile_id`）|
