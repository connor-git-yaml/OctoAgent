...
| `requested_agent_profile_id` / `requested_agent_profile_version` | str / int | 申请 Root Agent profile（`agent_profiles` kind=worker）+ revision（F117 D2 WorkerProfile 类已并入 AgentProfile；F117 W3 字段由 `requested_worker_profile_*` 收敛改名，事件 metadata key 读端双轨兼容老持久化）|
| `effective_profile_snapshot_id` | str | effective config snapshot（F117 W3 由 `effective_worker_snapshot_id` 改名）|
