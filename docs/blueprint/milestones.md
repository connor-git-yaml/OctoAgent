...
|---------|-----|--------|--------|------|
| **F137 门禁止血** ✅ 完成（2026-07-11，worktree `F137-gate-triage` 待拍板合入；Codex 3 轮 2P1+4P2 全闭环 0 HIGH；4877 passed 0 净回归 + e2e_smoke 8/8 + e2e_full 真跑 opt-in 实证）| gate | **P0** | 已交付：①硬闸 `model_request_gate`（env 缺省 allow 生产零感知；测试 pytest11 插件+根 conftest 双布线 deny，**显式 env 优先**）——闸点实测修正为 **`call()`+`embed()` 入口第一行**（`_dispatch` 太晚：OAuthResolver.resolve 是 preemptive 刷新会先打真 auth 端点）+ 5 处 swallow 站点 re-raise（fallback/llm_service×2/runner/memu_bridge embed 两级），deny 全量 triage 45 失败全为「直测 dispatch 机器」类②按文件声明放行，0 假绿 0 漏 opt-in；②CI 改写 B-lite 双 job（确定性层串行+junit+--reruns 1 过渡桥 / 前端 complexity+vitest 阻断）；③前端门禁进 pre-commit（SKIP_FRONTEND_CHECK=1）+3 阈值放宽 F143 ratchet；④marker 名实相符。**顺带抓 2 真 bug**：memu_bridge 4×`log`→NameError 潜伏（#6 违反）+ 前端 vitest 存量 11 failed 从未进闸（6 文件 --exclude 记欠账已派 fix chip，含疑似真 UI 回归）| S-M |
