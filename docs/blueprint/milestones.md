...
| **F141 三模式 lane 门禁** | gate | P1 | pr/baseline/release 三模式 + **release 强制 live**（skip 即 FAIL，堵 AC-1 真机验证反复推迟）+ change-policy 路径→check 路由 + `tests/AGENTS.md` 机器可读契约 + flaky quarantine 过期强制复查 + **coverage changed-lines 90% 门**（Fable 复审从 F142 挪入——gate 基建归 gate 体系）+ **人工验收 attestation lane**（手机/真机类物理不可自动化 AC 以签署清单进 release gate，而非假装可自动跑或漏掉）。依赖 F137 | M |
