...
| **F130 安全远程触达**（Tailscale）✅ 完成（2026-07-06，Codex 6 轮 + Opus 自审 0 HIGH/0 P1，20 P2+1 P3 全闭环（第 7 轮 Codex 撞用量上限→Opus 补位），4690 tests 0 regression + e2e_smoke 8/8）| **P0** | 手机经 Tailscale 私网（WireGuard）安全访问完整 Web UI。①`tailscale_helper` 三态（检测/建议/接管）+ DI exec **零 sudo**（serve `--bg --yes`，permission/HTTPS 未启用给手动指引不代跑）；②`octo remote enable/disable/status` 一键切 front_door.mode=bearer（**硬约束**：serve 从 loopback 代理会注入 X-Forwarded-*，loopback 模式会全 403，故必配 bearer 纵深）+ serve 成功才持久化（原子）+ 只关本功能映射（`--https=443 off` 不清整机 serve）；③**host↔mode 防裸奔校验**（纯函数 §E 矩阵 + 启动期 `exit(78)` fail-fast，`0.0.0.0+loopback`=裸奔拒绝，默认 `127.0.0.1+loopback`=safe）；④doctor 2 check（tailscale 连通 + 暴露面）；⑤log_redaction 补 tskey 前缀。**认证零重造**（复用 FrontDoorGuard，Constitution #10）+ tskey 零落盘（#5）。详见 `.specify/features/130-tailscale/completion-report.md` + `docs/codebase-architecture/remote-access.md` | M |
...
| **F140 L1 UI E2E** ✅ 完成（2026-07-12，worktree `F140-l1-playwright` 5 commits 待拍板；Codex spec 评审 0 finding + 双评审 0 HIGH；L1 3 场景三连绿 9.4s + 后端全量 0 regression）| L1 | P1 | 已交付：①`create_app(harness_factory=...)` DI 缝（生产不传构造性不可达，F087/F138 同范式）+ `l1_support/serve_l1_gateway.py` hermetic 启动器（零真 LLM 三重防御=gate deny/空凭证/resolve bomb）+ prompt-marker 脚本脑（长驻 server 跨测试，队列版会 desync）；②两场景（拍板③薄输入+外部断言）：chat→脚本决策环→`filesystem.write_text` 真写盘→SSE 回复渲染（真 EventSource 路径），bearer FrontDoorGate 输 token→SSE query 鉴权全链路+storage 模式断言；③断言全在 UI 外（REST 事件链+文件系统逐字节+wire task_id）+ cc-haha 失败 marker 定性扫描；④testid 契约（`e2e/selectors.ts` 单一事实源 + vitest 机械校验 7 锚点）；⑤CI 独立 `l1-playwright` job（零 secret，chromium 缓存，失败 trace artifact）。**场景②首跑抓 1 真 production bug**：api/client.ts `{headers,...init}` 展开序错误→调用方自带 headers 时 Authorization 被覆盖→bearer 模式聊天必 401（F130 手机远程一直留手工验证的路径），已修+回归用例。**spike 否证归档**：审批场景无确定性触发器（F136 execution context chat inline 不绑 / IRREVERSIBLE cron.delete 实测 broker 默认放行）→ deferred v0.2 | L |
| **F141 三模式 lane 门禁** ✅ 完成（2026-07-13，worktree `F141-lanes` 待拍板合入；spec 评审 Codex 3H+2M+1L 闭环 + 双评审 0 HIGH）| gate | P1 | 已交付六件：①`repo-scripts/lane.py` 三模式编排（pr=pre-commit hook canonical / baseline=合 master 前全量 / **release 强制 live**——live-real-llm lane「exit 0 且 passed≥1 且 unexpected_skip=0」三重判定 skip 即 FAIL、`SKIP_E2E` 无效、`--skip` 拒 live/attestation lanes、`octo attest service→remote` 解析 --json **status 字段**【service not_enabled 恒 FAIL / remote 默认 FAIL+`--allow-not-enabled` 降 WARN，堵「忘了部署还以为验过」】+ attestation 签署核对 `--require-signed` ≤90 天）+ **新 marker `real_llm`**（真打**事实**子集 2 文件级 + delegation 域#8 函数级【Codex final H1 抓漏补标】；lane 切分不复用 e2e_full——Codex spec H1：7/9 域文件确定性，复用会让确定性 PASS 掩护 live 假绿）②**flaky quarantine manifest**（`octoagent/tests/quarantine.json` 六字段 + `check-quarantine.py` **过期即门禁 FAIL** + 根 conftest 定向 `flaky(reruns=1)`；CI `--reruns 1` 过渡桥已删【F142 回收点兑现】；e2e_live blanket 收窄 e2e_full-only=live 变异性政策；F142 两性能断言**保 skipif 不入册**带理由——永久豁免无 exit criteria，入册会把过期强制复查污染成例行盖章）③change-policy 保守两条（纯 docs fastpath 跳 e2e+前端【sync-check 恒跑】+ missing-test WARNING 不阻断 + gate 机器资产 staged 附加校验）④**changed-lines coverage 门**（CI：裸 `--cov` 走 pyproject source 9 目录 + lcov∩diff 新增行 ≥90%、`[cov-exempt]` 大声记录、存量不背债；**e2e_scripted 16 用例 CI 专属步**回收 F142 budget-guard 遗留；pr lane 纳入 scripted 实测 24 用例 ~8s 远低 180s 预算）⑤`octoagent/tests/AGENTS.md` 机器可读测试契约（四层判定表/marker 语义表/lane 模式表/调用纪律/flaky 三分处置/红线/验证路由表/coverage 门；CLAUDE.md·testing-strategy·e2e-testing 三处指针化不复制）⑥门禁脚本自身单测 `tests/gate/`（quarantine/attestation/changed-lines/lane 编排 100+ 用例，cc-haha「门禁脚本必须被测试」教义）。制品 `.specify/features/141-three-mode-lanes/` | M |
...
| **F144 验收自动吸收**（2026-07-12 用户拍板新增：「手工验证项必须被分层吸收」）✅ 完成（2026-07-12，spec 评审 Codex 2P2 闭环 + 双评审 0 HIGH；4926=baseline 4872+54 0 regression + gap-1 2 case e2e_scripted 域全绿）| L3+live | P1 | 已交付：①frontdoor **mode×header L3 矩阵** 17 格补齐（guard 聚焦轻量 app；核心缺格「bearer+正确 token × 5 个 proxy hint header 逐个 → 200」把 F130「serve+bearer 可行」从人工复核记录变机械格；loopback×5 header/错 token±XFF/trusted_proxy 非 CIDR+错共享 header/SSE query token±XFF/query token 越界路径；`_PROXY_HINT_HEADERS` 契约钉住）②**`octo attest` 探针**（独立命令组不进 CI）：remote=mode==bearer 为 enabled 信号（**bearer+tailscale 断链=fail 非 not_enabled**，Codex spec P2-1）→ token 只从实例 .env 读值（P2-2）→ /ready+SPA+bearer 纵深+SSE 真握手；service=SIGKILL 真 pid→poll 恢复→新 pid（**显式偏离任务书 SIGTERM**：launchd SuccessfulExit=false+uvicorn 优雅 exit 0 → SIGTERM 必假失败）；三态 pass/not_enabled/fail（exit 0/0/1）+ --json 给 F141；36 hermetic 单测（DI 全 fake+sentinel 泄漏扫描 4 场景含 SSE 流中异常+dry-run 零 kill 机械断言；4 轮 Codex 6P2 全闭环回归钉住）③gap-1 **L3 scripted**：`test_e2e_scripted_write_approval.py`（e2e_scripted）——脚本脑×F136 审批全链走真 REST POST /api/approve，approve 落盘/reject 不写双路径，permission_preset=full 下审批仍触发=不可绕过最强断言④`attestation-checklist.md`（机器可读 YAML，仅 ATT-129-BOOT+ATT-130-PHONE optional）。handoff `.specify/features/144-attestation-absorb/handoff-to-F141.md`（探针契约+lane 编排+4 语义坑）| S-M |

**波②③落地 + 组合态验收（2026-07-12，本地叠加未 push 等拍板）**：F140 L1（抓 1 手机 bearer 401 真 bug 已修 + 派 SPA 遮蔽 webhook chip）∥ F142 护栏（3 依赖 bug 真库钉住 + CI backend `-n auto` 本地 ~14x）∥ F144 吸收（frontdoor 17 格矩阵 + `octo attest` remote/service 探针 + gap-1 L3 scripted + attestation 清单）三路并行，各双评审 0 HIGH、各自 0 regression。串行 rebase 叠加（F142→F140→F144，仅 testing-strategy 一处手解、CI workflow 两 job 自动并存、余自动合并）→ **组合态联合全量 5060 passed / 0 failed + e2e_smoke 8 + e2e_scripted 16（keystone+gap-1 存活）+ attest/frontdoor 62**。教训复现：三 agent 并行各自 commit 触发 hook uv-sync 把共享 venv 指向最后一个 worktree（F142-guards）→ 组合态验收前必主仓 `uv sync` 重指 + 无 dist 态跑（详见 memory precommit_hook）。**闭环（2026-07-12）**：①已 push（`2bd8679b..51422673`，SSH）；②**CI 首跑全绿**——frontend 32s / **l1-playwright 1m38s（新 L1 job 首跑即过，chromium+2 真浏览器场景）** / backend-deterministic **5m11s（`-n auto` 并行翻转生效，快于前 6m25s）**；③**真机 AC-1 自动化验收**：`octo attest service` 真跑 SIGKILL pid=10394→launchd 拉起 pid=86851→/ready 恢复 = **F129 崩溃自愈通过**（物理残余仅重启验开机自启）；`octo attest remote` = **not_enabled**（诚实报告：本机 mode=loopback+tailscale 未装，F130 远程链路代码在但未部署，探针给启用三步）。**M9 波②③收口。chip 双修复落地（2026-07-12，chip session 死前自行 push+CI 绿）**：`55e33748` SPA catch-all 遮蔽 lifespan 路由（webhook 405 雷）挂载迁 lifespan + `1c401cc8` F079 build-id 漂移检测恒误判"未构建"。**波④启动（2026-07-12）**：F141 lane ∥ F143 UI 变薄 ∥ F139 wire 回放 三路并行（文件面不冲突：gate 编排 / frontend / provider tests），M9 全部 Feature 进入实施。
...
**波④ CI 闭环 + M9 收官（2026-07-13）**：push `d22378b8..43e9a7c8`（SSH）。CI 首跑两轮 triage（**均为 F141/F142 门禁机制首次实战暴露、非本波回归，本地组合态 5214 恒绿**）：①F141 删 blanket `--reruns` 桥 → F083 已知 task_runner/pipeline 状态机异步时序 race + context_compaction 时序 **诚实暴露** → 按机制文件级入册 `quarantine.json` 3 条（定向 reruns=1 + 2026-08-13 强制复查 + 退出条件=F083 根因治理注入 clock/条件轮询；reruns 仅护偶发不掩盖确定性回归）；②release lane `live-real-llm` 假 FAIL——`-m real_llm` rootdir 收集触发 lib_semantics piper `importorskip`（缺可选依赖）module 级 collection-skip 先于 marker 过滤污染 junit → 收集路径限定 `e2e_live/` + 守卫测试锚定（真打 GPT-5.5 8/8 passed）→ **CI 全绿**（l1-playwright 1m13s / frontend 32s / backend 9m1s）。**`lane.py release` 真机 live 半边实跑**（主 session 替用户）：quarantine/complexity/backend/vitest/attest-service（SIGKILL 自愈 pid→pid PASS）全绿，attest-remote WARN（远程未部署 --allow-not-enabled 放行），**唯一 FAIL=attestation-signed ATT-129-BOOT 开机自启从未签署（设计内诚实 FAIL，物理残余=用户重启验一次签字，非缺陷）**。**M9 = M9 十 Feature（F137 硬闸/CI + F138 脚本化决策环 + F139 wire 回放 + F140 L1 + F141 lane + F142 护栏 + F143 UI 变薄 + F144 验收吸收）全清；四层金字塔从「倒挂+CI 断链」到「L1-L4 齐备+三模式 lane+真机验收自动化」。**
...
**验证吸收原则（2026-07-12 用户拍板，M9 全局约束）**：任何「请用户手工验证」的输出视为体系缺陷——必须先尝试分层吸收（L4 单元 → L3 确定性 e2e/scripted → L1 UI 自动化 → 本机 live 探针），只有物理不可自动化的残余才允许落 attestation 清单一行（F141 release lane 消费）。首批吸收=F144（F129/F130 AC-1 + F135 gap-1）。
...
| **F134 bearer 加固** ✅ 完成（2026-07-19，worktree `F134-bearer` 待拍板；Codex spec 0 finding + 双评审 0 HIGH；F144 17 格零触碰 + 新增限流矩阵 9 格/limiter 单元 7/remote 4/脱敏 4/attest 1）| S | 已交付三件：①认证失败限流（`_FailureRateLimiter` 收敛 FrontDoorGuard 内，#10）——**verify-first 语义**：60s 窗 10 次带错凭证失败→lockout 300s 期间错误凭证 429+Retry-After，**正确凭证恒放行+清计数**（serve 场景全远程共享 127.0.0.1 桶，锁定式会 DoS 唯一用户；TCP 层 key 不用可伪造 XFF；loopback 源不豁免、loopback 模式不接）；attest 负向断言 {401,429} 防限流期假阴性 ②`octo remote enable` 强 token 自动生成——未设时 `token_urlsafe(32)` 写实例 .env（0600，追加保内容），写失败不切 mode，**stdout 零明文**（删除 F130 明文建议打印，红线翻转归档）③SSE query token 泄露收敛选 (b)：取证唯一实锤面=uvicorn access log 绕过 F129 脱敏链落盘→`uvicorn.access` logger 级脱敏 filter + redaction 契约钉住；Referer/history 面否证、Tailscale 面理论归档；**ticket 化 (a) 完整设计存 spec §6 备 v0.2**（触发=走出私网）。前端零改动（与 F145 零交集）。制品 `.specify/features/134-bearer-hardening/` + remote-access.md §5c | ① |
...
| **F147 清扫篮** ✅ 完成（2026-07-19，8 commits 未 push 待拍板；Codex spec 2H+5M+3L 全闭环 + Codex final + Opus 自审 0 HIGH；5419 passed = baseline 5409 + 10 新测试 0 regression）| S-M | 已交付 5 项独立小项：①**toolsets 死配置清理**（删 toolsets.yaml + toolset_resolver.py + VALID_ENTRYPOINTS + list_for_entrypoint wrapper 4 无契约死项；`toolset`/`entrypoints` 字段经 Codex 查=plugin 公共 schema 故保留+标注，退役的是配置过滤层非字段）②**cron 三姊妹后台失败 HIGH 通知**（F102/F127/F111 genuine 失败[FAILED + spawn_error]补 HIGH；notification.py +`record_when_filtered` 参数——深夜 quiet hours 不推 channel 但入全局收件箱次日 Web 可发现；幂等键 run_id/date；manual notify=False 不重复推）③**bench infra 两债**：T1-TOOL-CALL-003 改读 README+JSON 去 AmbientRuntime 泄漏捷径 + 收紧 tool_name_contains:read / runner.py 429 指数退避 + 退避回滚 tracker.steps 不吃 max_steps 预算 + 上界复用 max_attempts ④**容器交付评估**=不做容器（deployment-and-ops.md §12.1.5 归档，取代 §12.1.2 Docker Compose 愿景；launchd/Tailscale 已覆盖）⑤**console_output 窄终端适配**（option b：探测 width<120 给 floor，覆盖非 TTY + 真 80 列 SSH，长 CJK 指引行完整）。制品 `.specify/features/147-cleanup/` | ② |
...
| **F150 Cloudflare 零信任远程** ⏳ 设计定稿待实施（2026-07-20，spec/plan/research/design-forks + 6 拍板收窄已 push master 8447a507，待 Codex+Opus 双评审后实施）| M-L | **认证三层纵深**：Cloudflare Access（边缘身份 OTP/SSO）→ gateway 验 `Cf-Access-Jwt`（不盲信边缘，PyJWT+JWKS 动态拉取）→ bearer（复用 F134 限流/强 token）。新增 `mode=cloudflared` 与 Tailscale 并存不加旁路（#10）；cloudflared 回源带 XFF 必走 bearer 纵深不裸 loopback。**排除 trycloudflare**。**用户拍板**：①gateway 验 JWT 做纵深 ②配对码=8 位短码换 bearer（TTL 10min/一次性）③cloudflared 检测+指引不全自动（免存 CF API token，#5）④用户 CF dashboard 手配 named tunnel+Access；⑤**SSE-over-GET Cloudflare 缓冲风险=方案 A**（Cloudflare 管可达+零信任，实时多轮任务流走 Tailscale E2E，写入 spec 验收 gate）；⑥**用户有 CF 托管域名**（硬前提满足，直接推进）。8 Phase：config→Access JWT 校验器→接 guard→cloudflared_helper→配对码→CLI→attest 全链→双评审 | ① |
