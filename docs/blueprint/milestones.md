...
**M6 债务/测试候选（穿插 F117-F122）**：F117 WorkerProfile/AgentProfile 合并（D2）✅ 核心完成（6b7ad8a3，W0-W4：删 WorkerProfile/Revision 类+表 + 塌缩 worker_profile_id + migration_117 真实例副本 dry-run/apply 验证 + 双评审；**W3 FE 改名 spin-out 未做**：requested_worker_profile_id ~104 wire + 19 持久化 metadata key + 前端）；F118 control_plane D8 解耦（M，并入 F108）；F119 e2e_live 补全 ✅（79caa367，F104/F116/F123/F124 + SSRF↔扫描互补链）；F120 F104 versionable 收窄 + FK 诚实化（L，**M7**）；F121 巨型 domain service 二次拆分（L，并入 F108）；F122 A2A docstring + worker_type 命名（S，并入 F108）。
