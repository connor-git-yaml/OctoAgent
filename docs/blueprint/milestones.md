...
| **F147 清扫篮** | S-M | toolsets 死配置清理（F-CLEAN-1）+ cron 后台失败 HIGH 通知 + bench infra 两债（T1-TOOL-CALL-003 断言收敛 / 并发限流指数退避跳出步数预算）+ 容器交付评估结论归档（预期不做容器，写清理由）+ **console_output 窄终端适配**（F134 CI triage 发现：模块单例 console width import 时锁死，非 TTF/窄终端 word-wrap 折断长 CJK 提示行——`octo remote enable` 等关键指引在 80 列 SSH 可读性差；评估非 TTY→plain 或 width 每次读环境）| ② |
