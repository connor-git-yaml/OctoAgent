...
| **F144 验收自动吸收**（2026-07-12 用户拍板新增：「手工验证项必须被分层吸收」）| L3+live | P1 | 把挂在用户身上的真机手工验收吸收进体系：①frontdoor **mode×header L3 矩阵**补格（bearer×XFF/5 个 proxy hint header/错 token——审计已标缺格，吸收 F130 AC-1 语义半边）②**本机 live 探针**（`octo` 命令或 e2e live 域，仅真机 opt-in 跑不进 CI）：remote 探针=tailscale→serve→curl published URL 带 token→断言 /ready+SPA+SSE（Mac 自身在 tailnet，吸收 F130 AC-1 链路半边）+ service 探针=status→kill 真 pid→poll /ready→新 pid（吸收 F129 AC-1 崩溃自愈）③F135 gap-1 的 **L3 scripted 用例**（F138 脚本脑驱动 behavior.write_file+审批全链）④残余 attestation 清单文件（仅「重启机器验开机自启」+ optional 手机首屏）。handoff 给 F141（探针即 release lane 的 live 件）| S-M |
