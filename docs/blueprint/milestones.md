...
完成Access登录、真实OpenAI OAuth doctor、Web对话、SSE与`SUCCEEDED`事件链；Web会话
过期、主动登出、重新登录、一次性恢复及Settings状态同步仍缺，继续作为当前
