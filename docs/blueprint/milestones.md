...
| **F139 provider wire 录制回放**（Fable 复审收窄，原「VCR 体系 L2→L3」）| L3 | P1→**P2** | **收窄为 provider transport 层 wire 真样本回归**：三 transport 各录少量真实 cassette（vcrpy 挂 ProviderClient `http_client` 注入点）+ **secret 过滤 serializer 先行**（#5，OAuth bearer/account_id 必洗）+ cassette 完整消费护栏。**不再承担「把 L2 agent-loop 用例降层」**——agent-loop 请求体含全量 system prompt/recall（高挥发），cassette body 匹配必常断、松匹配则假绿（pydantic-ai 自家教义：cassette 对 body 不敏感、wire shape 须单测钉住）；决策环降层由 F138 承担。依赖 F137 | S-M |
