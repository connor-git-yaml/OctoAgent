...
| Feature | 规模 | 一句话 | 波次 |
|---------|------|--------|------|
| **F145 统一候选审批中心** ✅ 完成（2026-07-19，6 commits；Codex spec 0 finding + final 0 HIGH/2 P2 全闭环 + Opus 对抗自审；5366 passed = baseline 5356+10 0 regression / 前端 vitest 412 / L1 4 passed）| M | 已交付：新「审批」nav 项（三源合计红点 badge，`GET /api/approval-center/summary` 只读汇总端点）+ `/approvals` **单页三分组**（非 tab：新记忆复用 CandidateCard 零改动 / F127 记忆合并 ProposalCard+来源预览 / F111 规则精简 unified diff 折叠渲染复用 DiffLineList），旧 `/memory/candidates` 页吸收删除 + redirect。**零改三源审批语义**；仅两处薄读扩展（summary 计数 + consolidation list `source_previews`——破坏性 MERGE 的知情决策材料，敏感候选/含敏感源恒空列表与审批端第三层防御同判定源）。CONFLICT 终态呈现按 body.status 分流（conflict=移除+已失效 toast 不诱导重试 / pending=保留+可重试）。**L1 场景③未 defer 真兑现**：launcher 注入 source_hash 对账候选 → UI 点接受 → 盘上覆写+pending 归零+bomb 三通道外部断言（F140 deferred 的审批场景闭环）。制品 `.specify/features/145-approval-center/` | ① |
