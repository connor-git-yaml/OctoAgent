# 架构审计记录（2026-04—2026-07）

> 本文件是 [blueprint.md](../blueprint.md) 架构审计章节的完整记录。
...
- D2 `AgentProfile + kind` 字段加入（F090 阶段 **WorkerProfile 类完全保留**，完全合并推迟）→
  **D2 已由 F117 完全闭合**：`WorkerProfile` / `WorkerProfileRevision` 类 + `worker_profiles` /
  `worker_profile_revisions` 表 + `AgentRuntime.worker_profile_id` 列全部物理删除，worker 配置并入
  `AgentProfile(kind=worker)` + `agent_profile_revisions`；migration_117 处理存量实例（kind 列
  ALTER+回填 / 表 DROP / 列塌缩 / 孤儿 backfill，schema-lag PRAGMA 驱动）。**F117 W3 收尾**：
  `works` 三列（`requested_worker_profile_id`/`_version`/`effective_worker_snapshot_id`）+ 事件
  metadata key + FE DTO 收敛改名为 `requested_agent_profile_id` 等；DB 列走 sqlite_init 数据保全
  防御 RENAME，事件 metadata key 读端双轨兼容（normalize alias 老持久化 key）。详见
  `.specify/features/117-profile-merge/`。
...
## 14.14 M11 前架构复审（2026-07-20）

> 本轮以当前工作树为准，不把旧审计结论当事实。输入包括：Octo 全仓 Spectra 4.3.0 AST 图、Ruff 复杂度审计、文件规模与 git churn，以及固定 SHA 的 OpenClaw、Hermes Agent、Agent Zero 源码审计。Spectra 的跨模块方向报告无法完整识别当前 Python monorepo，因此依赖方向结论均由静态 import 与 `pyproject.toml` 人工复核；竞品结论是源码审计，不等同于完整运行验收。

### 结论：保持模块化单体，先收口边界，不做内核重写

Octo 当前的产品边界仍然合理：单用户、单 Gateway application host、SQLite 事实源、ProviderRouter、Web-first、Cloudflare 单一公网入口。三个竞品都证明，过早增加 runtime、tunnel provider、Memory backend、Plugin SDK 和进程拓扑，会把兼容层、安全模型与测试矩阵一起放大。

本轮不新增 `apps/kernel`、`workers/*`、`packages/management` 等实体，也不以“大拆包”替代真实边界修复。`apps/gateway` 应在文档中诚实定义为 application host / modular monolith；只有出现第二个独立部署单元且有真实隔离收益时，才讨论物理拆分。

### 当前必须处理的问题

| 优先级 | 发现 | 证据与影响 | 收敛方向 |
|---|---|---|---|
| P0 | `provider/dx → gateway` 反向依赖仍存在，旧 §14.8 A2 的“已修复”结论已失效 | 当前工作树中 `packages/provider/src` 有 15 个生产文件、31 条 import 语句指向 `octoagent.gateway`；同时 Gateway 的包依赖又包含 Provider。它依赖 monorepo 可编辑安装掩盖运行时环，破坏 clean-wheel 契约 | 不建新 management 包；将产品 CLI/DX 迁入既有 `apps/gateway/cli`，Provider 只保留模型 transport/auth 能力，并增加 import-direction gate |
| P0 | LiteLLM 退役不完整 | Provider 仍硬依赖 `litellm[proxy]`，配置、health、备份、安装与 SDK 仍保留 `.env.litellm`、proxy/no-op 兼容路径；Blueprint/实现级文档仍把 Proxy 写成现役 | 直接删除无效兼容与 SDK 平行调用链；Provider 差异只留在一个 canonical turn/transport contract 的 adapter 内 |
| P0 | Docker 配置与实现不一致 | `docker_mode=required` 可配置，但生产 backend 只有 Inline/Graph；`required` 只会报“backend unavailable” | 删除或诚实重命名该配置。没有真实隔离 backend 前，不宣称 Docker sandbox，也不为模仿竞品重建容器实体 |
| P1 | application host 内部出现 God Service / 隐式全局注入 | `task_service.py` 3017 行、`orchestrator.py` 2782 行、`octo_harness.py` 1754 行；`AgentContextService` 使用 class-level setter 注入运行服务 | 保留现有 facade，按后续触达的垂直切片拆出职责；用 typed runtime service bundle 取代 class-level setter；禁止 big-bang 重写 |
| P1 | 数据与网络契约中心化 | `sqlite_init.py` 1969 行且含 40 个 DDL 常量；前端 `types/index.ts` 2021 行、186 个导出类型，是 Spectra 最大 hub；core 有 239 个 Pydantic 模型 | SQLite 迁移改为编号 registry；网络 DTO 由 OpenAPI 生成，手写类型只保留 UI view model；不再向单一 `types/index.ts` / `models/__init__.py` 堆叠 |
| P1 | 文档事实漂移 | Blueprint 仍混有 LiteLLM Proxy、物理 kernel/workers、Docker sandbox 等未实现或已退役描述；旧审计也存在“已修复”后再次漂移 | F151 同步 constitution、Blueprint、实现级文档与打包事实，并以静态门禁防止退役词和逆向 import 回流 |

复杂度扫描共报告 666 个热点信号（266 个参数过多、168 个高圈复杂度、90 个分支过多、75 个语句过多、67 个 return 过多）。这些不是 666 个功能 bug，但与文件规模和 churn 交叉后，`task_service.py`、`provider_client.py`、`orchestrator.py`、`task_runner.py`、`skills/runner.py` 是最应建立“不再恶化”ratchet 的区域。

### 竞品对照后的取舍

| 项目（审计 SHA） | 借鉴 | 明确不借 |
|---|---|---|
| [OpenClaw `fef846a`](https://github.com/openclaw/openclaw/tree/fef846a3f0e25b16602dda0677374a7e73c7432b) | 窄 Agent Loop、session lane、durable ingress、设备 challenge、method scope、Skill snapshot、架构 ratchet | VPN-first/host-first、全局 Scheduler、默认关闭 sandbox、shared secret=完整 operator、庞大 Plugin/Memory 兼容面 |
| [Hermes Agent `f0aae14`](https://github.com/NousResearch/hermes-agent/tree/f0aae14c684a84cd1eeca88339238406c30f3ed7) | tool-call 先持久化再执行、异常补齐 tool result、ProviderProfile、能力交集、clean-env 测试 | God loop、进程级工具注册表、失败回退 host local、无限子 Agent 总预算、“Memory 是权威数据”；被 `.skillignore` 排除扫描的文件仍被安装也是明确反例 |
| [Agent Zero `d1d48bc`](https://github.com/agent0ai/agent-zero/tree/d1d48bc9c0e6e253e87c354ce757c518820c6e25) | 项目包、任务取消/回收、Memory 人工管理、mobile/PWA、versioned state snapshot | 双 Tool pipeline、全局 Context、任意 AOP hook、默认启用同进程 Python Plugin、FAISS 事实源、root 容器、匿名 quick tunnel |

### M11-M12 的架构硬约束

1. **单一主路径**：一个 Agent turn contract、一个 Tool execution pipeline、一个 Task/Event 事实链；Channel、Web、Routine 不复制 Agent Loop。
2. **运行态显式持有**：Scheduler、registry、LLM/runtime service 由 application runtime 持有；禁止新增进程级可变 registry 或 class-level service injection。
3. **默认拒绝**：identity → ownership → action scope → tool capability → approval 逐层判断；安全配置或执行 backend 不可用时 fail closed，禁止回退本机高权限执行。
4. **Memory 不是指令**：SQLite/结构化记录是事实源，向量索引是可重建 projection；召回内容带 provenance、scope、time、confidence、consent/revocation，并按不可信证据处理。
5. **扩展闭包等于扫描闭包**：可安装/可执行文件不得借 ignore 规则绕过扫描；没有第二个真实实现前，不开放通用 Plugin/Context/Memory backend SDK。
6. **公网晚于边界**：F150 可以先做只读真实链路 spike，但生产公网入口必须通过 F151 的依赖、打包、runtime truth 与 fail-closed 门禁。

对应实施顺序与完成门禁见 [milestones.md](milestones.md) 的 M11-M12。
