...
- 适用场景：当前长时工具以及未来可能新增的隔离/远程执行；Docker、SSH backend 尚未实现
...
`terminal.exec` 的 cwd 受同样策略保护。命令级过滤由 ToolBroker、PathAccessPolicy 与审批链承担；当前没有 Docker sandbox，不能把过滤责任留给未实现的后端。
...
### 8.8 Execution Plane：Gateway 内逻辑 Worker + Runtime Backend
...
- 全局策略决策（由同一 Gateway application host 内的 Policy Engine 负责）
...
class RuntimeBackend(Protocol):
    ...
    async def execute(
        self, *, task_service, envelope, llm_service, execution_context, cancel_signal
    ) -> None: ...
```

当前 backend：
- `inline`：默认，调用 Gateway 内的 TaskService 执行。
- `graph`：真实消费 pydantic-graph 的确定性编排路径。
- Docker / SSH / remote GPU：均未实现；`required` 配置只能 fail closed，不得回退到 Inline 后宣称隔离成功。

#### 8.8.3 隔离策略边界

- 当前没有可用的 Docker、SSH 或远程 sandbox backend。
- 安全边界由 ToolBroker、Policy Engine、PathAccessPolicy、审批与 runtime budget 共同执行。
- 对宿主机的操作必须通过专用 tool，并默认 ask（除非白名单）。
- 未来若新增隔离 backend，必须提供真实进程/文件系统/网络边界及独立验收，不得用 Inline 路径兼容或伪装。
...
| **dismiss 跨通道统一 + 跨重启持久化（F116）** | Telegram callback button 或 Web API `POST /api/notifications/{id}/dismiss` 任一通道 dismiss 后，通过共享 `_dismissed_set` 让另一端下次查询（`GET /api/notifications` / `list_active`）不再展示；Telegram 已推送消息不撤回（仅 Web 端会过滤掉）。**F116**：dismiss + active 元数据落 `notification_dismissals` / `notification_active` 两表（`SqliteNotificationStore`，挂 `StoreGroup` 主连接），bootstrap `rehydrate()` 恢复——重启后已读通知不再重现（Constitution #1）；dismiss best-effort 落盘并经返回值/route `persisted` 字段诚实暴露降级（DB 故障不升级为 500）；rehydrate 同时 seed `_notified_set` 防重启后重复推送；session/task 级联删除清 `notification_active` 防 stale 通知复活 |
...
- ~~dismiss 跨重启持久化~~ **已由 F116 实现**（见上「dismiss 跨通道统一 + 跨重启持久化」）
- API 字段 `FR-D4` 显式参数化
...
1. **cron 触发**：APScheduler 按 USER.md 的 `daily_summary_time` 字段（如 `09:00`）触发；生效时区按 USER.md `user_timezone` 机器可读字段（F115）> `OCTOAGENT_USER_TIMEZONE` env（部署兜底）> `UTC` 降级（SD-10 时区语义，全程 UTC 归一化）。F115 前时区只读 env，USER.md 改时区对 cron 触发时刻 + "昨日"窗口零影响，已修
...
**USER.md 4 个机器可读字段**（F115 起，原 3 个 + `user_timezone`）：
...
| `user_timezone`（F115）| 机器可读 IANA 时区名（如 `Asia/Shanghai`）；缺失/非法降级 env `OCTOAGENT_USER_TIMEZONE` → `UTC`。与人类可读"时区/地点"字段并存 |
...
- dismiss 持久化（已由 F116 实现，原推迟 F107）
