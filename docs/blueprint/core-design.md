...
| **Quiet hours** | USER.md SoT 解析（如 `22:00-07:00`），命中即 **discard 而非 enqueue**（避免堆积 backlog）；过滤事件仍写 EventStore |
| **dismiss 跨通道统一** | Telegram callback button 或 Web API `POST /api/notifications/{id}/dismiss` 任一通道 dismiss 后另一端自动同步 |
| **去重 + dedupe** | `notification_id = sha256(category + target_id + content_hash)`；同 id 重复 emit 触发覆盖（不重复推送）|
| **`NOTIFICATION_DISPATCHED` EventType** | 每条 notification 写 EventStore（含 quiet hours 内被过滤的）；payload 含 priority / channels_dispatched / channels_filtered / dispatch_status |
