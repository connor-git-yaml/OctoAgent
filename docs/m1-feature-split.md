...
| Agent Zero `python/helpers/mcp_handler.py` + `prompts/agent.system.mcp_tools.md` | MCP 工具原生集成（MCPTool 包装 + 工具注入提示） | 未集成 MCP | M1 Feature 007 |
...
#### v5 竞品源码深度分析（Agent Zero 循环控制 + OpenClaw 防护）

##### Agent Zero 核心模式

| 模式 | 源码位置 | 机制 | 借鉴价值 |
|------|----------|------|---------|
| 双层循环 | `agent.py` monologue() | 外层 monologue loop + 内层 message_loop，`break_loop` 信号终止内层 | 高 — SkillRunner 需要类似的「Skill 完成信号」 |
| break_loop 终止 | `models/response.py` | Tool 返回 `Response(break_loop=True)` 终止当前 loop | 高 — OutputModel 中加 `skip_remaining_tools` 字段 |
| 三层异常处理 | `agent.py` monologue() | InterventionException → 暂停等审批；RepairableException → 重试修复；Generic → 报告失败 | 高 — SkillRunner 需区分可恢复/不可恢复错误 |
| SKILL.md 加载 | `python/tools/skills_tool.py` | Worker 通过 skills_tool 动态加载 SKILL.md 到 EXTRAS 上下文 | 中 — 可考虑 Skill 文档注入机制 |
| Extension 钩子 | `agent.py` 15+ hook 点 | monologue_start/end、message_loop_start/end、before/after_llm_call、tool_execute_before/after 等 | 高 — SkillRunner 生命周期需要类似可扩展钩子 |
| dirty_json 解析 | `helpers/dirty_json.py` | 容错 JSON 解析 prompt 中的工具调用 | 低 — Pydantic AI structured output 已解决此问题 |

##### OpenClaw 防护模式

| 模式 | 机制 | 借鉴价值 |
|------|------|---------|
| 4 型循环检测 | tool-loop-detection：检测重复调用签名、死循环、振荡模式 | 高 — SkillRunner 必须有循环检测 |
| Context Budget Guard | tool-result-context-guard：工具返回结果超出 context 预算时截断 | 中 — 避免单个工具输出撑爆上下文窗口 |

##### 五项优化建议

1. **循环终止信号** — OutputModel 中加 `complete: bool` 或 `skip_remaining_tools: bool`，Skill 完成时主动通知 SkillRunner 停止
2. **重复调用检测** — hash 每轮 tool_calls 签名，连续 3 次相同签名时触发告警并终止
3. **三级异常分流** — SkillRepeatError（可重试）、SkillValidationError（需修复输入后重试）、ToolExecutionError（记录并报告）
4. **生命周期钩子** — skill_start/end、before/after_llm_call、before/after_tool_execute，供 observability 和自定义拦截
5. **结构化工具结果** — ToolResultMessage 包含 artifact_ref、is_error、metadata，而非纯文本
...
#### v5 竞品源码深度分析（Policy Pipeline + Approval 架构）

##### OpenClaw Policy Pipeline（最高借鉴价值）

| 模式 | 源码 | 机制 | 借鉴价值 |
|------|------|------|---------|
| 7 层 Cascade 策略 | `applyToolPolicyPipeline` | GLOBAL → PROJECT → AGENT → SESSION → TOOL_TYPE → TOOL_INSTANCE → RUNTIME 逐层覆盖 | 高 — 简化为 3 层（GLOBAL → WORKSPACE → AGENT）已足够 |
| Label 决策追踪 | 每层附带 label | `{ decision: "allow", label: "workspace:allowlist" }` 可追溯决策来源 | 高 — 审计必备 |
| consumeAllowOnce 原子操作 | `exec-approval-manager.ts` | 原子性消费一次性审批令牌，避免竞态条件 | 高 — 直接采用 |
| 15s Grace Period | 审批通过后 15s 有效窗口 | 避免用户反复审批相同类型操作 | 中 — 可配置宽限期 |
| 幂等注册 | 相同 ID 不重复添加审批队列 | Map-based 去重 + pruneExpiredRequests 定期清理 | 高 — 防止重复审批弹窗 |

##### AgentStudio Pre-Send Guard（有借鉴也有反面教训）

| 模式 | 机制 | 价值 |
|------|------|------|
| Provider Pipeline | 有序 Provider 链，每个返回 allow/block/rewrite/require_confirm | 高 — 可插拔策略评估管道 |
| Rewrite 决策 | 策略可重写工具参数后放行（如敏感参数脱敏） | 中 — M2 可考虑 |
| `require_confirm` 未实现 | 定义了枚举值但 switch 分支为空 | **反模式** — 枚举必须全部实现或不定义 |
| Provider 链无 fast-fail | block 后仍可能继续评估后续 Provider | **反模式** — block 应立即短路返回 |
| Provider 配置与能力不匹配 | `onError: "continue"` 可能跳过安全类 Provider | **反模式** — 应根据 Provider 类型定义 failureStrategy |

##### Agent Zero 干预系统（反面教材为主）

| 模式 | 机制 | 价值 |
|------|------|------|
| handle_intervention() | `asyncio.sleep(0.1)` 轮询等待用户响应 | **反模式** — 应使用 asyncio.Event 事件驱动 |
| 非持久化状态 | 干预状态仅在内存中 | **反模式** — 审批状态必须持久化到 Event Store |

##### 前端审批 UI（OpenClaw 最完整）

| 设计 | 机制 | 借鉴 |
|------|------|------|
| 三按钮决策 | Allow Once / Always Allow / Deny | 高 — 直接采用此 UX 模式 |
| 队列 Badge | 显示 `"3 pending"` 排队数量 | 中 — Telegram 渠道可用 inline keyboard |
| 倒计时过期 | 每个请求独立过期倒计时 | 中 — 避免无限等待 |

##### 三项架构优化建议

1. **3 层 Policy Cascade** — GLOBAL → WORKSPACE → AGENT 优先级覆盖，每层决策携带 label 追踪来源。比 OpenClaw 7 层简单，保留核心思想
2. **Two-Phase Approval 增强** — 采用 consumeAllowOnce 原子操作 + 可配置宽限期 + Event Store 持久化（非内存），审批状态变更通过 SSE 推送
3. **Provider Pipeline + Fast-Fail** — 策略评估管道支持 allow/block/request_approval 三种决策，block 立即短路返回

##### 必须避免的三个反模式

| 反模式 | 来源 | OctoAgent 对策 |
|--------|------|---------------|
| 审批状态非持久化 | Agent Zero | 所有审批记录写入 Event Store，进程重启后可恢复 |
| 轮询等待审批结果 | Agent Zero `asyncio.sleep(0.1)` | 使用 `asyncio.Event` + SSE 事件驱动 |
| 枚举值部分实现 | AgentStudio `require_confirm` | Python 用 exhaustive match + `assert_never()` 确保所有分支覆盖 |
...
**当前状态（v6）**：进行中（Ready for Review）

- 已完成：
  - Phase 1-3 复盘优化（补齐 product-research + research-synthesis + 方案评审）
  - 真实联调测试落地：`octoagent/tests/integration/test_f007_e2e_integration.py`
  - 关键回归：004/005/006 + 002/003 关键能力均通过
- 待完成：
  - 将 `feat/007-e2e-m1-integration` 合并到 `master`
  - 按项目策略决定是否在 M1.5 启动主链路 SkillRunner 化重构
...
| 来源 | 借鉴能力 | 007 中的应用 |
|------|---------|-------------|
| Agent Zero `mcp_handler.py` + `agent.system.mcp_tools.md` | MCP 工具原生注册为一等工具（发现 → 包装 → 调用） | 已补齐参考路径；007 仅完成本地工具链联调，MCP 原生注册顺延后续里程碑 |
...
- [x] LLM 调用 → 结构化输出 → 工具执行 端到端通过
- [x] irreversible 工具触发审批流，approve 后继续执行
- [x] 工具 schema 自动反射与代码签名一致（contract test 通过）
- [x] Blueprint §14 M1 核心验收标准（007 范围内）通过
...
| **AgentZero** | `python/helpers/mcp_handler.py` + `prompts/agent.system.mcp_tools.md` | 007（MCP 工具原生集成） |
...
| **AgentZero** | `python/agent.py` monologue() 双层循环 | 005（SkillRunner 循环控制） |
| **AgentZero** | `python/models/response.py` Response.break_loop | 005（Skill 完成信号） |
| **AgentZero** | `python/helpers/dirty_json.py` | 005（反面参考：Pydantic AI 无需此方案） |
| **AgentZero** | `python/tools/skills_tool.py` SKILL.md 加载 | 005（Skill 文档注入） |
| **OpenClaw** | `src/agents/tool-loop-detection.ts` 4 型循环检测 | 005（SkillRunner 循环检测） |
| **OpenClaw** | `src/agents/tool-result-context-guard.ts` | 005（Context Budget Guard） |
| **OpenClaw** | `src/gateway/exec-approval-manager.ts` consumeAllowOnce | 006（原子审批消费） |
| **OpenClaw** | `src/ui/views/exec-approval.ts` 三按钮决策 UI | 006（前端审批 UX） |
| **OpenClaw** | `src/ui/controllers/exec-approvals.ts` 队列管理 | 006（审批队列去重 + 过期清理） |
| **AgentStudio** | `backend/src/types/preSendGuard.ts` 4-decision 枚举 | 006（反面参考：exhaustive check 教训） |
