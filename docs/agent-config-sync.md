...
- 同步脚本：`repo-scripts/sync-agent-config.sh`
...
2. 若有平台差异：编辑对应模板头部文件
3. 运行同步：`./repo-scripts/sync-agent-config.sh`
4. 漂移检查：`./repo-scripts/sync-agent-config.sh --check`

可选本地私有配置同步（不入库）：

- `./repo-scripts/sync-agent-config.sh --sync-local`
- 行为：`CLAUDE.local.md` -> `AGENTS.local.md`

## 约束

- 不要手改 `CLAUDE.md` / `AGENTS.md`（两者为生成文件）
- 如果要上 CI，直接加一条检查命令：`./repo-scripts/sync-agent-config.sh --check`
