...
- **向开发者要决策时说大白话**：需要我（开发者）拍板时，问题和每个选项都用大白话写清楚、给足背景——从产品/用户视角说明这个选择会影响用户什么（功能能不能用、体验差别、有没有风险或代价），不要只摆技术方案、内部字段或学术名词。不要假定我已经看过你提到的代码和文档，做判断需要的关键信息直接写进问题和选项里。原因：脱离用户场景的技术化提问让人无法权衡利弊，等于把决策成本甩回给我、还容易选错。例外：问题确实复杂到几句话讲不清时，先给一句话结论 + 推荐项，再附"想深入可看 X 文档/代码"的指引，不要用"你去看代码就懂了"代替解释
