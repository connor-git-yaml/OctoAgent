...
from octoagent.gateway.services.operations.channel_verifier import VerifierAvailability
...
class TestFormatReport:
    ...
    async def test_format_guidance_returns_panel_when_findings(self, tmp_path: Path) -> None:
        ...
        runner = DoctorRunner(project_root=tmp_path)
        report = await runner.run_all_checks(live=False)
        guidance_panel = format_guidance_panel(build_guidance(report))
    ...
    async def test_build_guidance_ignores_optional_dotenv_checks_when_yaml_exists(
        ...
        report = await runner.run_all_checks(live=False)
        guidance = build_guidance(report)
        check_names = {item.check_name for group in guidance.groups for item in group.items}
...
class TestDoctorCli:
    ...
    def test_doctor_exit_code_follows_blocking_guidance(
        ...
        from click.testing import CliRunner
        from octoagent.gateway.services.operations import doctor as doctor_module
        ...
        monkeypatch.setattr("octoagent.gateway.cli.cli._resolve_project_root", lambda: Path.cwd())
