...
from octoagent.gateway.cli import behavior_commands as bc
...
class TestTrigger:
    ...
    def test_trigger_specific_file_with_project(self, runner, monkeypatch):
        fake = _FakeRequest([(200, {"run_id": "r", "proposals_made": 0, "outcomes": []})])
        monkeypatch.setattr(bc, "_compact_request", fake)
        monkeypatch.setattr(bc, "_compact_resolve_project_slug", lambda ref: ref or "selected-proj")
        result = runner.invoke(bc.behavior_group, ["compact", "PROJECT.md", "--project", "myproj"])
        assert result.exit_code == 0, result.output
    ...
    def test_trigger_project_file_resolves_selected_project(self, runner, monkeypatch):
        ...
        fake = _FakeRequest([(200, {"run_id": "r", "proposals_made": 0, "outcomes": []})])
        monkeypatch.setattr(bc, "_compact_request", fake)
        monkeypatch.setattr(bc, "_compact_resolve_project_slug", lambda ref: "selected-proj")
...
class TestDecide:
    def test_apply_200(self, runner, monkeypatch):
        fake = _FakeRequest([(200, {"ok": True, "status": "applied", "file_id": "AGENTS.md"})])
        monkeypatch.setattr(bc, "_compact_request", fake)
    ...
    def test_apply_409_conflict_guides_retrigger(self, runner, monkeypatch):
        fake = _FakeRequest([(409, {"ok": False, "status": "conflict", "detail": "源文件已变更"})])
        monkeypatch.setattr(bc, "_compact_request", fake)
        result = runner.invoke(bc.behavior_group, ["compact", "--apply", "cand-1"])
        assert result.exit_code != 0
...
class TestGuards:
    def test_modes_mutually_exclusive(self, runner):
        result = runner.invoke(bc.behavior_group, ["compact", "--list", "--apply", "cand-1"])
        assert result.exit_code != 0
    ...
    def test_trusted_proxy_mode_fails_fast(self, runner, monkeypatch, tmp_path):
        ...
        import octoagent.gateway.services.operations.service_manager as sm
...
class TestListSize:
    def test_list_size_local_measurement(self, runner, monkeypatch, tmp_path: Path):
        ...
        monkeypatch.setattr(bc, "_compact_resolve_project_slug", lambda ref: ref or "myproj")
