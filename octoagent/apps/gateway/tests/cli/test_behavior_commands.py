...
def test_behavior_init_creates_project_files(tmp_path: Path) -> None:
    ...
    assert "Storage Boundaries" in (target_dir / "instructions" / "README.md").read_text(
        encoding="utf-8"
    )
    assert '"bindings": []' in (project_root / "project.secret-bindings.json").read_text(
        encoding="utf-8"
    )
...
def test_behavior_ls_and_show_report_effective_sources(tmp_path: Path) -> None:
    ...
    # 使跨行截断的路径片段能被完整匹配。
    flat_output = "".join(line.strip().strip("│╭╰─").strip() for line in listed.output.splitlines())
...
def test_behavior_agent_scope_uses_requested_agent_slug(tmp_path: Path) -> None:
    ...
    init_result = runner.invoke(
        main, ["behavior", "init", "--scope", "agent", "--agent", "Research Root Agent"], env=env
    )
...
def test_behavior_agent_scope_uses_distinct_hash_slug_for_non_ascii_agent_names(
    ...
    agent_dirs = sorted(
        path.name for path in (tmp_path / "behavior" / "agents").iterdir() if path.is_dir()
    )
