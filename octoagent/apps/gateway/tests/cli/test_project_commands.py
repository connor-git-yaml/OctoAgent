...
def test_project_edit_wizard_status_and_apply(tmp_path: Path, monkeypatch: Any) -> None:
    class RecordingSetupAdapter:
        def __init__(self, project_root: Path) -> None:
            self._project_root = project_root

        async def prepare_wizard_draft(self, draft: dict[str, Any]) -> dict[str, Any]:
            return dict(draft)

        async def review(self, draft: dict[str, Any]) -> SimpleNamespace:
            _ = draft
            return SimpleNamespace(data={"review": {"ready": True}})

        async def apply(self, draft: dict[str, Any]) -> SimpleNamespace:
            save_config(OctoAgentConfig.model_validate(draft["config"]), self._project_root)
            return SimpleNamespace(code="SETUP_APPLIED", message="配置已应用")

    monkeypatch.setattr(
        "octoagent.gateway.services.operations.setup_governance_adapter."
        "LocalSetupGovernanceAdapter",
        RecordingSetupAdapter,
    )
    runner = CliRunner()
    env = {"OCTOAGENT_PROJECT_ROOT": str(tmp_path)}
    ...
    assert "draft_secret_targets=1" in started.output
...
def test_provider_package_declares_gateway_dependency(tmp_path: Path) -> None:
    ...
    _ = tmp_path
    pyproject = Path(__file__).resolve().parents[4] / "packages/provider/pyproject.toml"
