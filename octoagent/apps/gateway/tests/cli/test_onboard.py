...
from octoagent.gateway.services.operations.secret_models import SecretAuditReport
...
def _bootstrapper(project_root: Path, *, echo: bool = False):
    from octoagent.gateway.services.config.config_bootstrap import ConfigBootstrapResult
...
async def test_onboarding_service_surfaces_secret_audit_detail(
    ...
    monkeypatch.setattr(
        "octoagent.gateway.services.operations.onboarding_service.SecretService.audit",
        _fake_audit,
    )
