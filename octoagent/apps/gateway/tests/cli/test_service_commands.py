...
from octoagent.gateway.cli import service_commands
from octoagent.gateway.cli.service_commands import (
    _resolve_log_file,
    _tail_lines,
    logs_command,
    service_group,
...
class FakeServiceManager:
    ...
    def install(self, *, dry_run: bool, force: bool, keep_awake: bool):
        self.install_calls.append({"dry_run": dry_run, "force": force, "keep_awake": keep_awake})
...
def _patch_manager(monkeypatch: pytest.MonkeyPatch, fake: FakeServiceManager) -> None:
    monkeypatch.setattr(service_commands, "build_service_manager", lambda _root: fake)
...
def test_install_and_uninstall_require_source_checkout_before_side_effects(
    tmp_path: Path,
    ...
    _write_source_descriptor(source_instance, source_root)
    _write_source_descriptor(wheel_instance, tmp_path / "installed-wheel")

    accepted = FakeServiceManager(
        install_result=ServiceInstallResult(backend="launchd", action="installed"),
        uninstall_result=ServiceInstallResult(backend="launchd", action="uninstalled"),
    )
    _patch_manager(monkeypatch, accepted)
    monkeypatch.setattr(service_commands, "resolve_instance_root", lambda: source_instance)
    runner = CliRunner()
    assert runner.invoke(service_group, ["install"]).exit_code == 0
    assert runner.invoke(service_group, ["uninstall"]).exit_code == 0

    rejected = FakeServiceManager(
        install_result=ServiceInstallResult(backend="launchd", action="installed"),
        uninstall_result=ServiceInstallResult(backend="launchd", action="uninstalled"),
    )
    _patch_manager(monkeypatch, rejected)
    monkeypatch.setattr(service_commands, "resolve_instance_root", lambda: wheel_instance)
    install_result = runner.invoke(service_group, ["install"])
    uninstall_result = runner.invoke(service_group, ["uninstall"])
    failures: list[str] = []
    for label, result in (("install", install_result), ("uninstall", uninstall_result)):
        if result.exit_code != 69:
            failures.append(f"{label} exit={result.exit_code}")
        if "SOURCE_CHECKOUT_REQUIRED" not in result.output:
            failures.append(f"{label} missing typed error")
    if rejected.install_calls or rejected.uninstall_calls:
        failures.append("service manager side effect reached")
    assert not failures, "F151_SERVICE_SOURCE_GUARD_MISSING: " + "; ".join(failures)
...
class TestServiceInstallCli:
    def test_install_success(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert fake.install_calls == [{"dry_run": False, "force": False, "keep_awake": False}]
    ...
    def test_install_flags_forwarded(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert result.exit_code == 0
        assert fake.install_calls == [{"dry_run": True, "force": True, "keep_awake": True}]

    def test_install_repair_required_exits_nonzero(self, monkeypatch: pytest.MonkeyPatch) -> None:
        fake = FakeServiceManager(
            install_result=ServiceInstallResult(
                backend="launchd",
                action="blocked",
    ...
    def test_unsupported_platform_friendly_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestServiceUninstallStatusCli:
    ...
    def test_status_not_installed_gives_install_hint(self, monkeypatch: pytest.MonkeyPatch) -> None:
        fake = FakeServiceManager(status_result=ServiceStatus(backend="launchd", installed=False))
        _patch_manager(monkeypatch, fake)
        result = CliRunner().invoke(service_group, ["status"])
        assert result.exit_code == 0
...
class TestLogsCli:
    ...
    def test_logs_falls_back_to_service_stderr_on_startup_crash(self, log_env: Path) -> None:
...
class TestTailAcrossRotation:
    def test_tail_spans_rotated_files(self, tmp_path: Path) -> None:
        ...
        # 轮转序：.2（最老）→ .1 → base（最新）
        (tmp_path / f"{PROCESS_LOG_FILE}.2").write_text("old-a\nold-b\n", encoding="utf-8")
        (tmp_path / f"{PROCESS_LOG_FILE}.1").write_text("mid-a\nmid-b\n", encoding="utf-8")
