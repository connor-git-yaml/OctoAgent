...
def test_parse_full_config() -> None:
    ...
    F151 cleanup：退役 runtime 输入仍在 raw YAML 阶段拒绝，但根模型不再保留空块。
...
def test_default_values() -> None:
    ...
    F151 cleanup：根模型不再暴露空 ``runtime`` 字段。
    """
    config = OctoAgentConfig(updated_at="2026-03-04")
    assert config.config_version == 1
    assert config.providers == []
    assert config.model_aliases == {}
    assert "runtime" not in config.model_dump(mode="python")
...
def test_from_yaml_rejects_exact_retired_runtime_keys_before_model_validation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    model_validation_called = False

    def reject_model_validation(cls: type[OctoAgentConfig], payload: object) -> None:
        del cls, payload
        nonlocal model_validation_called
        model_validation_called = True
        raise AssertionError("model validation ran before raw runtime tombstone")

    monkeypatch.setattr(
        OctoAgentConfig,
        "model_validate",
        classmethod(reject_model_validation),
    )
    for key in ("llm_mode", "litellm_proxy_url", "master_key_env"):
        yaml_text = f'updated_at: "2026-03-04"\nruntime:\n  {key}: retired\n'
        try:
            with pytest.raises(ConfigParseError) as exc_info:
                OctoAgentConfig.from_yaml(yaml_text)
        except AssertionError:
            pytest.fail(RAW_RUNTIME_TOMBSTONE_ORACLE, pytrace=False)
        error = exc_info.value
        if error.field_path != f"runtime.{key}" or "RUNTIME_CONFIG_RETIRED" not in error.message:
            pytest.fail(RAW_RUNTIME_TOMBSTONE_ORACLE, pytrace=False)
    if model_validation_called:
        pytest.fail(RAW_RUNTIME_TOMBSTONE_ORACLE, pytrace=False)


def test_from_yaml_removes_empty_runtime_and_preserves_v1_v2_provider_inputs(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    v1_yaml = """
updated_at: "2026-03-04"
...
"""
    ...
    original_model_validate = OctoAgentConfig.model_validate

    def capture_model_validation(cls: type[OctoAgentConfig], payload: object) -> OctoAgentConfig:
        del cls
        if not isinstance(payload, dict):
            pytest.fail(RUNTIME_SCHEMA_NORMALIZATION_ORACLE, pytrace=False)
        validated_payloads.append(payload.copy())
        return original_model_validate(payload)

    monkeypatch.setattr(
        OctoAgentConfig,
        "model_validate",
        classmethod(capture_model_validation),
    )
    try:
        v1 = OctoAgentConfig.from_yaml(v1_yaml)
        v2 = OctoAgentConfig.from_yaml(v2_yaml)
    except ConfigParseError:
        pytest.fail(RUNTIME_SCHEMA_NORMALIZATION_ORACLE, pytrace=False)
    expected = (
        len(validated_payloads) == 2,
        all("runtime" not in payload for payload in validated_payloads),
        v1.providers[0].api_base == "https://openrouter.example/v1",
        v1.providers[0].effective_auth_kind == "api_key",
        v1.providers[0].effective_api_key_env == "OPENROUTER_API_KEY",
        v2.providers[0].transport == "anthropic_messages",
        v2.providers[0].api_base == "https://anthropic.example/v1",
        v2.providers[0].effective_auth_kind == "oauth",
        v2.providers[0].effective_oauth_profile == "anthropic-profile",
    )
    if not all(expected):
        pytest.fail(RUNTIME_SCHEMA_NORMALIZATION_ORACLE, pytrace=False)

    with pytest.raises(ConfigParseError) as exc_info:
        OctoAgentConfig.from_yaml('updated_at: "2026-03-04"\nruntime:\n  future_runtime: enabled\n')
    if (
        exc_info.value.field_path != "runtime.future_runtime"
        or "RUNTIME_CONFIG_UNKNOWN" not in exc_info.value.message
    ):
        pytest.fail(RUNTIME_SCHEMA_NORMALIZATION_ORACLE, pytrace=False)
