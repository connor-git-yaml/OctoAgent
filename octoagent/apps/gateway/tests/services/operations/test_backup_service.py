...
async def test_plan_restore_checks_first_existing_ancestor_for_writability(
    ...
    monkeypatch.setattr(
        "octoagent.gateway.services.operations.backup_service.os.access", fake_access
    )
...
async def test_export_chats_includes_session_continuity_payload(tmp_path: Path) -> None:
    ...
    assert payload["agent_session_turns"]["agent-session-turn-001"]["tool_name"] == "memory.search"
