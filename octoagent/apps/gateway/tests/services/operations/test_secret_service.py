...
from octoagent.gateway.services.operations.secret_status_store import SecretStatusStore
from octoagent.gateway.services.operations.update_status_store import UpdateStatusStore


def test_secret_status_store_roundtrip_corruption_and_atomic_replace(tmp_path: Path) -> None:
    first = SecretStatusStore(tmp_path, project_id="project-1")
    second = SecretStatusStore(tmp_path, project_id="project-1")
    initial = SecretApplyRun(
        run_id="run-1",
        project_id="project-1",
        status="running",
    )
    completed = initial.model_copy(update={"status": "completed"})
    materialization = RuntimeSecretMaterialization(
        snapshot_id="snapshot-1",
        project_id="project-1",
        resolved_env_names=["OPENROUTER_API_KEY"],
    )

    first.save_apply(initial)
    first.save_apply(completed)
    first.save_materialization(materialization)

    assert second.load_apply() == completed
    assert second.load_materialization() == materialization

    apply_path = tmp_path / "data" / "ops" / "projects" / "project-1" / "secret-apply.json"
    apply_path.write_text("{not-json", encoding="utf-8")
    assert first.load_apply() is None
    assert apply_path.with_suffix(".json.corrupted").read_text(encoding="utf-8") == ("{not-json")
    assert list(apply_path.parent.glob("*.tmp")) == []


def _write_secret_test_config(project_root: Path) -> None:
    """写入 Provider 直连所需的最小 secret 配置。"""
    save_config(
        OctoAgentConfig(
...
async def test_secret_service_reload_managed_runtime_and_redaction(
    ...
    apply_payload = json.loads(
        (
            tmp_path / "data" / "ops" / "projects" / "project-default" / "secret-apply.json"
        ).read_text()
    )
...
async def test_secret_service_warning_only_bridge_reports_ready(tmp_path: Path) -> None:
    """canonical .env 中的 Provider key 可作为显式 legacy bridge。"""
    _write_secret_test_config(tmp_path)
