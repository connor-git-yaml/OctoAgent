...
from octoagent.gateway.services.operations.import_mapping_store import ImportMappingStore
from octoagent.gateway.services.operations.import_source_store import ImportSourceStore
...
from octoagent.memory import ImportInputRef, ImportMappingProfile, ImportSourceType


def _mapping_profile(*, mapping_id: str, updated_at: datetime) -> ImportMappingProfile:
...
def test_import_mapping_store_roundtrip_corruption_and_independent_instances(
    tmp_path: Path,
) -> None:
    first = ImportMappingStore(tmp_path)
    second = ImportMappingStore(tmp_path)
    now = datetime.now(tz=UTC)
    older = _mapping_profile(mapping_id="mapping-old", updated_at=now - timedelta(seconds=1))
    newer = _mapping_profile(mapping_id="mapping-new", updated_at=now)

    first.save(older)
    first.save(newer)

    assert second.get(newer.mapping_id) == newer
    assert (
        second.get_latest(
            project_id="project-1",
            workspace_id="workspace-1",
    ...
    corrupt_path = (
        tmp_path / "data" / "control-plane" / "imports" / "mappings" / f"{newer.mapping_id}.json"
    )
    corrupt_path.write_text("{not-json", encoding="utf-8")
    assert first.get(newer.mapping_id) is None
    assert [item.mapping_id for item in first.list()] == [older.mapping_id]
    assert list(corrupt_path.parent.glob("*.tmp")) == []


def test_import_source_store_roundtrip_atomic_and_corruption(tmp_path: Path) -> None:
    first = ImportSourceStore(tmp_path)
    second = ImportSourceStore(tmp_path)
    source = ImportSourceDocument(
        resource_id="source:source-1",
        active_project_id="project-1",
        active_workspace_id="workspace-1",
        source_id="source-1",
        source_type=ImportSourceType.WECHAT,
        input_ref=ImportInputRef(input_path="/isolated/export.json"),
    )
    run = ImportRunDocument(
        resource_id="run-1",
        active_project_id="project-1",
        active_workspace_id="workspace-1",
    ...
    assert second.get_source(source.source_id) == source
    assert second.get_run(run.resource_id) == run
    assert second.list_sources(project_id="project-1") == [source]
    assert second.list_runs(source_id=source.source_id) == [run]

    source_path = tmp_path / "data" / "control-plane" / "imports" / "sources" / "source-1.json"
    source_path.write_text("{not-json", encoding="utf-8")
    assert second.get_source(source.source_id) is None
    assert second.list_sources() == []
    assert list((tmp_path / "data" / "control-plane" / "imports").rglob("*.tmp")) == []
