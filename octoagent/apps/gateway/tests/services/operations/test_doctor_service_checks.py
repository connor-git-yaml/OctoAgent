...
class TestCheckServiceStatus:
    ...
    async def test_not_installed_warns_recommended_not_blocking(self, tmp_path: Path) -> None:
        runner = _runner_with_service(tmp_path, ServiceStatus(backend="launchd", installed=False))
        result = await runner.check_service_status()
    ...
    async def test_running_but_ready_false_warns_not_pass(self, tmp_path: Path) -> None:
    ...
    async def test_systemd_installed_without_linger_warns(self, tmp_path: Path) -> None:
        ...
        class LingerOffManager(FakeStatusManager):
            def __init__(self) -> None:
                ...
                self.backend = SystemdUserBackend(
                    ...
                    command_runner=lambda cmd, timeout: CommandOutcome(0, "Linger=no\n", ""),
                )
    ...
    async def test_installed_not_running_warns_with_repair_hint(self, tmp_path: Path) -> None:
        runner = _runner_with_service(
            tmp_path,
...
def _runner_with_sleep(tmp_path: Path, risk: SleepRisk) -> DoctorRunner:
    return DoctorRunner(
        project_root=tmp_path,
        service_manager_factory=lambda _root: FakeStatusManager(ServiceStatus(backend="launchd")),
...
class TestCheckSleepSettings:
    ...
    async def test_will_sleep_laptop_warns_with_honest_hint(
        ...
        monkeypatch.setattr("octoagent.gateway.services.operations.doctor.sys.platform", "darwin")
    ...
    async def test_linux_hint_has_no_mac_only_advice(
        ...
        monkeypatch.setattr("octoagent.gateway.services.operations.doctor.sys.platform", "linux")
        runner = _runner_with_sleep(
            tmp_path,
    ...
    async def test_sleep_disabled_laptop_still_mentions_lid(self, tmp_path: Path) -> None:
        runner = _runner_with_sleep(
            tmp_path,
...
class TestRunAllChecksIncludesNewChecks:
    async def test_new_checks_present_and_injected(self, tmp_path: Path) -> None:
        runner = DoctorRunner(
            ...
            sleep_risk_probe=lambda: SleepRisk(supported=True, will_sleep=True, is_laptop=True),
        )
        ...
        new_checks = [
            check for check in report.checks if check.name in ("service_status", "sleep_settings")
        ]
...
class TestProbeSleepRisk:
    ...
    def test_linux_battery_detection(self, tmp_path: Path) -> None:
        (tmp_path / "BAT0").mkdir()
        runner = FakeCommandRunner({})
        risk = probe_sleep_risk(runner, platform_name="linux", sys_power_supply=tmp_path)
        assert risk.supported is True
    ...
    def test_linux_no_battery(self, tmp_path: Path) -> None:
        (tmp_path / "AC").mkdir()
        risk = probe_sleep_risk(None, platform_name="linux", sys_power_supply=tmp_path)
        assert risk.is_laptop is False
...
def test_probe_never_calls_mutating_commands(platform_name: str, tmp_path: Path) -> None:
