...
| ~~5~~ | ~~`context_compaction.py:_tiktoken_encoder`~~ | ~~已剔除~~ | ~~lazy import 一次性 init，不被运行期写入；reset 反而破坏后续测试（``test_context_compaction.TestFallbackChain`` 整组挂）。F087 P2 T-P2-7 hermetic 验证后**剔除**~~ | ~~~~ | grep 1（已剔除） |
