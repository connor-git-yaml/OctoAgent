...
import click
from octoagent.gateway.cli.console_output import click_prompt_driver, create_console
...
def _save_memory_patch(
    ...
    *,
    ...
    if updated.providers:
        _print_saved_config_summary(updated)
    else:
        console.print("[dim]当前没有 Provider。[/dim]")


def _print_saved_config_summary(config: OctoAgentConfig) -> None:
    """输出已保存的 Provider 与 alias 摘要。"""
    try:
        ...
        console.print("  说明: 当前进程不会热重载；需要时请重启正在运行的 Gateway。")
    except Exception as exc:
...
def config(ctx: click.Context, yaml_path: str | None) -> None:
    """OctoAgent 统一配置管理；octoagent.yaml 是单一事实源。"""
...
def _show_summary(yaml_path: str | None) -> None:
    ...
    console.print()
    # F081 cleanup：原 Runtime 段（llm_mode / litellm_proxy_url / master_key_env）
    # 已随 LiteLLM Proxy 退役整体删除。
...
def config_init(
    ...
    try:
        ...
        result = bootstrap_config(
            project_root,
            echo=echo,
            prompt=lambda text, default: str(click_prompt_driver("field", text, default)),
            choice_prompt=lambda text, choices, default: str(
                click_prompt_driver("choice", text, default, choices)
            ),
        ...
        _print_saved_config_summary(result.config)
...
@click.option(
    "--base-url", default=None, help="自定义 API Base URL（如 https://api.siliconflow.cn/v1）"
)
...
def provider_add(
    ...
    # Feature 081 P4：写到 .env 而非 .env.litellm（旧配置已升级）
...
@click.option("--model", default=None, help="Provider 模型字符串")
