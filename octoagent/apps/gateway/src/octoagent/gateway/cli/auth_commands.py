...
from octoagent.gateway.services.config.config_bootstrap import build_canonical_reauth_config
...
from octoagent.provider.auth.validators import validate_claude_setup_token
...
def canonical_reauth_command[RecoveryResult](
    project_root: Path,
    action: Callable[[], RecoveryResult],
) -> RecoveryResult:
    """执行canonical reauth动作，不读取或迁移退役runtime文件。"""
    return build_canonical_reauth_config(
        project_root,
        build=action,
        on_legacy=lambda filename: click.echo(
            f"检测到退役配置文件 {filename}；recovery未读取或迁移其内容。",
            err=True,
        ),
    )
...
def paste_token(provider: str) -> None:
    ...
    canonical_reauth_command(
        Path(_resolve_project_root()),
        lambda: store.set_profile(profile),
    )
