...
def service_install(dry_run: bool, force: bool, keep_awake: bool, verbose: bool) -> None:
    ...
    resolve_managed_source_checkout(resolve_instance_root())
...
def _render_status(status: ServiceStatus, *, verbose: bool) -> None:
    ...
    if not status.installed:
        ...
        lines.append(
            "提示: 服务在运行但 /ready 未通过（gateway 当前不可用）——`octo logs` 查失败原因。"
        )
    if verbose:
    ...
    healthy = status.installed and status.loaded and status.running and status.ready is not False
    console.print(
        render_panel(
            "octo service status",
            lines,
...
def logs_command(lines: int, follow: bool, level: str | None, verbose: bool) -> None:
    ...
    if level is not None:
        ...
        if normalized not in _LEVEL_ORDER:
            raise click.ClickException(f"未知级别 `{level}`（可用: {', '.join(_LEVEL_ORDER)}）")
    ...
    if main_missing_or_empty:
        ...
        if stderr_has_content:
            ...
            return
    elif stderr_has_content and (stderr_fallback.stat().st_mtime > log_file.stat().st_mtime):
