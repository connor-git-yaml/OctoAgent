...
from octoagent.gateway.services.config.config_bootstrap import bootstrap_config
...
SOURCE_CHECKOUT_EXIT_CODE = 69
SOURCE_CHECKOUT_ERROR_CODE = "SOURCE_CHECKOUT_REQUIRED"


class SourceCheckoutRequiredExit(SystemExit):
    """源码托管命令在wheel-only环境中的稳定退出。"""

    def __init__(self) -> None:
        super().__init__(SOURCE_CHECKOUT_EXIT_CODE)

    def __str__(self) -> str:
        return SOURCE_CHECKOUT_ERROR_CODE


def find_source_checkout(candidate: Path) -> Path | None:
    """从候选目录向上查找真实OctoAgent Git checkout，并返回项目根。"""
    resolved = candidate.expanduser().resolve()
    for checkout in (resolved, *resolved.parents):
        project_root = checkout / "octoagent"
        required = (
            checkout / ".git",
            checkout / "benchmarks" / "runner" / "cli.py",
            project_root / "pyproject.toml",
            project_root / "uv.lock",
            project_root / "scripts" / "install-octo.sh",
        )
...
def require_source_checkout(candidate: Path) -> Path:
    """要求真实源码checkout；失败时在任何写操作前稳定exit 69。"""
    project_root = find_source_checkout(candidate)
    if project_root is not None:
        return project_root
    print(
        f"{SOURCE_CHECKOUT_ERROR_CODE}: 此命令需要OctoAgent源码checkout。",
        file=sys.stderr,
    )
    raise SourceCheckoutRequiredExit


def resolve_managed_source_checkout(runtime_root: Path) -> Path:
    """从managed descriptor解析源码根；无descriptor时使用当前目录。"""
    resolved_runtime_root = runtime_root.expanduser().resolve()
    descriptor = UpdateStatusStore(
        resolved_runtime_root,
        data_dir=resolved_runtime_root / "data",
    ).load_runtime_descriptor()
    if descriptor is None:
        descriptor = UpdateStatusStore(resolved_runtime_root).load_runtime_descriptor()
    candidate = Path(descriptor.project_root) if descriptor is not None else Path.cwd()
    return require_source_checkout(candidate)
...
def run_install_bootstrap(
    ...
    *,
    ...
    root = require_source_checkout(resolve_project_root(project_root))
    started_at = utc_now()
    ...
    descriptor_store = status_store
    descriptor = status_store.migrate_runtime_descriptor_for_install()
    if descriptor is None and instance_root is not None:
        descriptor_store = UpdateStatusStore(instance_root.expanduser().resolve())
        descriptor = descriptor_store.migrate_runtime_descriptor_for_install()
    ...
    try:
        ...
        if should_skip_runtime_bootstrap:
            ...
            attempt.runtime_descriptor_path = str(descriptor_store.descriptor_path)
            ...
            if instance_root is None:
                attempt.next_actions.extend(
                    [
                        ...
                        (
                            "使用 uv run python -m octoagent.gateway "
