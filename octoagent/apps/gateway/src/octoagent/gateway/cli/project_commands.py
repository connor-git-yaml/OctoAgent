...
def edit_project(
    ...
    async def _run() -> None:
        ...
        from octoagent.gateway.cli.console_output import click_prompt_driver

        wizard_service = WizardSessionService(root, prompt_driver=click_prompt_driver)
        if wizard_status:
                ...
                console.print(
                    render_panel(
                        ...
                        [
                            ...
                            *[f"next_action={item}" for item in review.get("next_actions", [])[:3]],
                        ],
                        border_style="yellow",
                    )
                )
