...
from octoagent.gateway.cli.attest_commands import attest_group  # 本机 live 验收探针
from octoagent.gateway.cli.auth_commands import auth
from octoagent.gateway.cli.backup_commands import backup, export, restore
from octoagent.gateway.cli.behavior_commands import behavior_group
from octoagent.gateway.cli.chat_import_commands import import_cmd
from octoagent.gateway.cli.cleanup_commands import cleanup_group  # Feature 082 P4
from octoagent.gateway.cli.config_commands import _resolve_project_root, config
from octoagent.gateway.cli.console_output import (
    click_prompt_driver,
    create_console,
    render_panel,
)
from octoagent.gateway.cli.memory_commands import memory  # F094 E1: octo memory migrate-094 命令组
from octoagent.gateway.cli.project_commands import project_group
from octoagent.gateway.cli.secret_commands import secrets_group
from octoagent.gateway.cli.service_commands import logs_command, service_group  # F129 常驻服务地基
from octoagent.gateway.cli.update_commands import restart, stop, update, verify
from rich.console import RenderableType

console = create_console()


def setup_recovery_composition[RecoveryResult](
    project_root: Path,
    build: Callable[[], RecoveryResult],
) -> RecoveryResult:
    """构造canonical setup状态，不读取或迁移退役runtime文件。"""
    from octoagent.gateway.services.config.config_bootstrap import (
        build_canonical_reauth_config,
    )

    return build_canonical_reauth_config(
        project_root,
        build=build,
        on_legacy=lambda filename: console.print(
            f"[yellow]检测到退役配置文件 {filename}；recovery未读取或迁移其内容。[/yellow]"
        ),
    )
...
def _build_setup_config_patch(config: Any) -> dict[str, Any]:
    return {
        ...
        "model_aliases": {
            alias: model.model_dump(mode="python") for alias, model in config.model_aliases.items()
        },
    }
...
main.add_command(attest_group)  # octo attest service 本机验收探针
...
def setup(
    ...
    """新手一键接入真实模型。"""
    from octoagent.gateway.cli.console_output import format_guidance_panel, format_report
    from octoagent.gateway.services.config.config_bootstrap import (
        build_bootstrap_config_for_provider,
        list_bootstrap_provider_choices,
    )
    from octoagent.gateway.services.operations.doctor import DoctorRunner, build_guidance
        ...
        if provider_choice == "custom":
            resolved_provider_id = (
                (
                    resolved_provider_id
                    or click.prompt(
                        "Provider ID（如 siliconflow / ollama）", default="custom-provider"
                    )
                )
                .strip()
                .lower()
            ...
            config = setup_recovery_composition(
                project_root,
                lambda: build_bootstrap_config_for_provider(
                    provider_choice,
...
def init(manual_oauth: bool) -> None:
    ...
    try:
        if manual_oauth:
            console.print(
                "[yellow]提示：统一 wizard 已接管 init，manual_oauth 标志仅保留兼容性。[/yellow]"
            )
        ...
        async def _run() -> None:
            ...
            wizard = WizardSessionService(root, prompt_driver=click_prompt_driver)
...
def onboard(channel: str, restart: bool, status_only: bool) -> None:
    """首次使用统一入口。"""
    from octoagent.gateway.cli.console_output import format_guidance_panel
    from octoagent.gateway.services.operations.onboarding_service import OnboardingService
    from octoagent.gateway.services.operations.telegram_verifier import (
        build_builtin_verifier_registry,
    )
