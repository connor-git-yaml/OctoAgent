...
from octoagent.gateway.services.operations.project_selector import (
...
def show_behavior(file_id: str, project_ref: str | None, agent_ref: str) -> None:
    ...
    async def _run() -> None:
        ...
        if selected is None:
            ...
            raise ProjectSelectorError(f"未找到 behavior file: {normalized}。可用文件: {known}")
        console.print(
            render_panel(
                "Behavior File",
                [
...
@click.option(
    ...
    show_default=True,
    help=(
        "优先 materialize 到 project shared / system shared / agent private / project-agent overlay"
    ),
)
...
def _resolve_behavior_edit_target(
    *,
    ...
    target_kind = selected.source_kind if selected.source_kind in compatible_kinds else default_kind
...
def _compact_gateway_settings() -> tuple[str, dict[str, str]]:
    ...
    - front_door bearer 模式：token 值只从实例 ``.env`` 读
    ...
    from octoagent.gateway.services.operations.service_manager import resolve_instance_root
...
def _compact_request(method: str, path: str, payload: dict | None = None) -> tuple[int, dict]:
    ...
    try:
        with httpx.Client(timeout=_COMPACT_HTTP_TIMEOUT_S, follow_redirects=False) as client:
            resp = client.request(method, f"{base_url}{path}", json=payload, headers=headers)
...
def compact_behavior(
    ...
    if sum(modes) > 1:
        raise click.ClickException("--apply / --reject / --list / --list-size 互斥，一次只能选一个")
...
def _compact_list_pending() -> None:
    ...
    status, body = _compact_request("GET", "/api/behavior/compact/candidates?limit=1000")
    if status != 200:
    ...
    for cand in candidates:
        ...
        console.print()
    console.print("落盘：octo behavior compact --apply <候选id>；拒绝：--reject <候选id>")
