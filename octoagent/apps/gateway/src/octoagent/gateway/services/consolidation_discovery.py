...
LLM 层留强 model 验证"；③``tool_profile="minimal"`` 当前挂不到任何工具（无 builtin 工具标 minimal），
free-loop 路径需先打通 per-tool profile override 链路（spec 第一决策点未拍板）。**不造 memory
原语**——propose/validate/commit 全复用 ``MemoryService`` 写管道；窗口读复用 ``search_sor``。
    ...
    def _build_discovery_prompt(self, facts: list[SorRecord]) -> str:
        ...
        for fact in ordered:
            ...
            if used_chars + len(entry) > LLM_INPUT_CHAR_BUDGET:
                lines.append(f"...（还有 {len(ordered) - included} 条事实未列出，本次不纳入）")
                break
            lines.append(entry)
