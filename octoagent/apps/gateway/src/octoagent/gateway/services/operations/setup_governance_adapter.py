...
from octoagent.gateway.services.operations.wizard_session import DEFAULT_SETUP_AGENT_PROFILE
...
class LocalSetupGovernanceAdapter:
    ...
    async def prepare_wizard_draft(self, draft: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(active_profile, Mapping) and str(active_profile.get("name", "")).strip():
            return prepared
