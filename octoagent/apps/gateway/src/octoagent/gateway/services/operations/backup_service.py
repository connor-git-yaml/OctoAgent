...
class BackupService:
    ...
    def _create_bundle_sync(
        ...
        with tempfile.TemporaryDirectory(prefix="octo-backup-") as tmp_dir:
            ...
            config_sources = {"config/octoagent.yaml": self._root / "octoagent.yaml"}
    ...
    def _default_bundle_filename(self, created_at: datetime, label: str | None) -> str:
        suffix = ""
        if label:
            normalized = "".join(ch.lower() if ch.isalnum() else "-" for ch in label.strip()).strip(
                "-"
            )
            if normalized:
                suffix = f"-{normalized}"
    ...
    def _serialize_agent_session(self, session: AgentSession) -> dict[str, Any]:
        ...
        return {
            ...
            "latest_compaction_summary": str(metadata.get("latest_compaction_summary", "")).strip(),
