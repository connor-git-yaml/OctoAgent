...
from octoagent.gateway.services.operations.secret_models import ResolvedSecretRef, SecretRef
from pydantic import SecretStr

SecretCommandRunner = Callable[
...
def _resolve_exec_secret(
    ref: SecretRef,
    *,
    environ: dict[str, str],
    cwd: Path,
    command_runner: SecretCommandRunner | None,
) -> ResolvedSecretRef:
    ...
    if command_runner is None:
        raise SecretResolutionError(
            "SECRET_EXEC_RUNNER_REQUIRED",
            "SecretRef(exec) 必须使用显式注入的 command runner。",
        )
    normalized_command = [str(item) for item in command]
    timeout_seconds = int(ref.locator.get("timeout_seconds", 10))
    returncode, stdout, stderr = command_runner(normalized_command, cwd, environ, timeout_seconds)
    if returncode != 0:
        raise SecretResolutionError(
            "SECRET_EXEC_FAILED",
            stderr.strip() or stdout.strip() or "exec 返回非零退出码。",
        )
    value = stdout.strip()
    if not value:
        raise SecretResolutionError("SECRET_EXEC_EMPTY", "exec 返回空 secret。")
    summary = f"exec:{' '.join(normalized_command[:2])}"
