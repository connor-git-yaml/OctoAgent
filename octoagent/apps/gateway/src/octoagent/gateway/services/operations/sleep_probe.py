...
def _probe_darwin(run: CommandRunner) -> SleepRisk:
    ...
    if outcome.ok:
        ...
        disable_match = re.search(r"^\s*SleepDisabled\s+1", outcome.stdout, flags=re.MULTILINE)
        if disable_match:
            will_sleep = False
...
def _probe_linux(sys_power_supply: Path) -> SleepRisk:
    ...
    try:
        has_battery = any(entry.name.startswith("BAT") for entry in sys_power_supply.iterdir())
    except OSError:
