...
_DESTRUCTIVE_WORKSPACE_SYNC_COMMAND = [
    "/bin/bash",
    "-lc",
...
def build_safe_workspace_sync_command() -> list[str]:
    return [
        "/bin/bash",
        "-lc",
        "GIT_ROOT=$(git rev-parse --show-toplevel 2>/dev/null) && "
        ...
        "    echo 'LOCAL_CHANGES_PRESENT' >&2; exit 75; "
        "  fi && "
...
def build_workspace_sync_command() -> list[str]:
    return build_safe_workspace_sync_command()
...
def normalize_runtime_descriptor(
    ...
    if descriptor.workspace_sync_command in (
        _LEGACY_WORKSPACE_SYNC_COMMAND,
        _DESTRUCTIVE_WORKSPACE_SYNC_COMMAND,
    ):
