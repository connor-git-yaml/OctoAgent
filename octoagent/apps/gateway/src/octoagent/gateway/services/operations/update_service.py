...
from octoagent.gateway.services.operations.doctor import CheckStatus, DoctorRunner
...
_GIT_STATUS_COMMAND = ["git", "status", "--porcelain=v1", "--untracked-files=all"]
...
def _workspace_has_local_changes(status: str) -> bool:
    return any(
        line.startswith("?? ")
        or (len(line) >= 2 and (line[1] not in {" ", "?"} or line[0] not in {" ", "?"}))
...
def _default_launch_worker(project_root: Path, attempt_id: str) -> None:
    ...
    subprocess.Popen(  # noqa: S603
        [
            sys.executable,
            "-m",
            "octoagent.gateway.services.operations.update_worker",
            "--project-root",
            str(project_root),
            "--attempt-id",
            attempt_id,
...
class UpdateService:
    ...
    async def apply(
        ...
        *,
        ...
        if descriptor is not None:
            self._ensure_workspace_clean(Path(descriptor.project_root).resolve())
        management_mode = (
    ...
    def _persist_attempt(self, attempt: UpdateAttempt) -> None:
        if attempt.overall_status in (
            ...
            self._release_owned_active_attempt(attempt.attempt_id)
        else:
            self._claim_or_update_active_attempt(attempt)
        self._status_store.save_latest_attempt(attempt)

    def _claim_or_update_active_attempt(self, attempt: UpdateAttempt) -> None:
        current = self._status_store.load_active_attempt_with_token()
        if current is None:
            token = self._status_store.try_claim_active_attempt(attempt)
            if token is not None:
                return
            current = self._status_store.load_active_attempt_with_token()
        if current is None:
            raise ActiveUpdateError(attempt.attempt_id)
        active, token = current
        if active.attempt_id != attempt.attempt_id:
            raise ActiveUpdateError(active.attempt_id)
        next_token = self._status_store.update_active_attempt(
            attempt,
            compare_token=token,
        )
    ...
    def _release_owned_active_attempt(self, attempt_id: str) -> None:
        current = self._status_store.load_active_attempt_with_token()
        if current is None:
            return
        active, token = current
        if active.attempt_id != attempt_id:
            return
        self._status_store.release_active_attempt(attempt_id, compare_token=token)
    ...
    async def _run_migrate_phase(
        ...
        self._ensure_workspace_clean(workspace_root)
    ...
    def _ensure_workspace_clean(self, workspace_root: Path) -> None:
        status = self._command_runner(_GIT_STATUS_COMMAND, workspace_root)
        if _workspace_has_local_changes(status):
            raise UpdateActionError(
                "LOCAL_CHANGES_PRESENT",
                "检测到本地工作区变更，已在执行任何更新命令前安全停止。",
                status_code=409,
                exit_code=75,
            )
    ...
    async def _run_restart_phase(
        ...
        if process.poll() is not None:
            raise RuntimeError(f"新进程启动后立即退出，exit code={process.poll()}。")
        phase.completed_at = utc_now()
        phase.status = UpdatePhaseStatus.SUCCEEDED
