...
class TelegramStateStore:
    ...
    def _mutate_state(self, mutation: Callable[[TelegramState], object]) -> TelegramState:
        with self._lock:
    ...
    def delete_approved_user(self, user_id: str | int) -> None:
        key = str(user_id)
        self._mutate_state(lambda state: state.approved_users.pop(key, None))
    ...
    def resolve_reply_thread_root(
        ...
        *,
        ...
        return self.load().reply_thread_roots.get(self._reply_thread_key(chat_id, message_id))
    ...
    def remember_reply_thread_root(
        ...
        *,
        ...
        state.reply_thread_roots[self._reply_thread_key(chat_id, message_id)] = root
        self.save(state)
        return root
    ...
    def upsert_pending_pairing(
        ...
        *,
        ...
        now = _now()
        keep_existing_code = (
            existing is not None and existing.status == "pending" and existing.expires_at > now
        )
        pending = TelegramPendingPairing(
            ...
            display_name=display_name or (existing.display_name if existing is not None else ""),
            requested_at=(
                existing.requested_at if keep_existing_code and existing is not None else now
            ),
            expires_at=(
                existing.expires_at
                if keep_existing_code and existing is not None
            ...
            status="pending",
            last_message_text=(
                last_message_text or (existing.last_message_text if existing is not None else "")
            ),
        )
    ...
    def set_polling_offset(self, offset: int | None) -> int | None:
        state = self._mutate_state(lambda current: setattr(current, "polling_offset", offset))
