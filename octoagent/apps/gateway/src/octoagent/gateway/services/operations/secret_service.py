...
from octoagent.gateway.services.operations.update_service import UpdateActionError, UpdateService
...
class SecretService:
    ...
    async def audit(self, project_ref: str | None = None) -> SecretAuditReport:
        ...
        async with self._store_group_scope() as store_group:
            ...
            binding_map = {
                (binding.target_kind, binding.target_key): binding for binding in bindings
            }
    ...
    async def reload(self, *, project_ref: str | None = None) -> SecretReloadResult:
        ...
        async with self._store_group_scope() as store_group:
            ...
            with self._temporary_env(
                {binding.env_name: secret.get_secret_value() for binding, secret in resolved_pairs}
            ):
