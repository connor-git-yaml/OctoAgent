...
2. **commit 前验证**（codex P1/P2 修复，``_verify_sources_for_commit``）：
   - 敏感防御第三层（P1 最后闸，前两层在发现端窗口排除 + _propose_group）：候选
     ``is_sensitive`` / 候选 partition 敏感 / 任一源 SOR partition 敏感 → 拒绝——
     敏感 MERGE commit 会被 ``_safe_sor_content`` 毁内容（content 换 rationale 且
     MERGE 不建 vault），v0.1 一律不 commit（v0.2 vault-aware MERGE 再放开）。
   - 源新鲜度（P2）：候选 pending 期间源 SOR 可能被更新/删除/被其他候选合并——
     逐源验证仍 ``current``；任一不 current → 拒绝，**绝不静默用旧内容 commit**
     （否则新 current 与合并事实并存 + supersede 打到已过期的行）。
   - 任一验证失败 → 候选 APPLYING→CONFLICT（终态，CAS）+ emit CONFLICTED
     （actor=SYSTEM，非用户决策）+ REST 409。不回滚 PENDING（候选内容已失效，
     重审也不能 commit，终态避免用户反复 accept 反复失败）。
3. ``write_service.commit_memory(proposal_id)``——commit MERGE（新权威事实 CURRENT + 源标
   SUPERSEDED 软删可回滚）。proposal 是 Phase C 已 VALIDATED 持久化的。
4. 状态 APPLYING→APPLIED（CAS expected_status）+ emit MEMORY_CONSOLIDATION_APPROVED。
5. 任何失败 → 回滚 APPLYING→PENDING（候选重新可审，复用 Memory Candidates rollback 范式）。
...
- C2 Everything-is-an-Event：accept→APPROVED / reject→REJECTED / 系统检测失效→CONFLICTED
  审计事件（PII 防护 id 引用）。
- C6 Degrade Gracefully：commit 失败 → 回滚到 PENDING 不崩（候选可重审）。
- NFR-3（codex P1 修复后语义）：敏感分区（HEALTH/FINANCE）候选 v0.1 **不可 commit**——
  发现端已不产（窗口排除 + 纵深防御），本服务是第三层最后闸（防存量/数据侧漏网）；
...
from octoagent.memory.enums import SENSITIVE_PARTITIONS, MemoryLayer, SorStatus
...
class ConsolidationApprovalService:
    ...
    async def accept(self, candidate_id: str) -> ApprovalResult:
        ...
        **C4 红线**：本方法是合并提议**唯一** commit 路径，且只被用户主动 accept 触发。
        ...
        Returns:
            ...
            status="not_found"（候选不存在）/ "conflict"（claim 失败已被处理/非 pending，
            **或** commit 前验证失败：源 SOR 已过期（P2）/ 敏感分区防御（P1）——此时
            候选已转 CONFLICT 终态，detail 说明原因）/
    ...
    # ============================================================
    # commit 前验证（codex P1 第三层 + P2）
    # ============================================================

    async def _verify_sources_for_commit(
        self, candidate: ConsolidationCandidate
    ) -> tuple[str, list[str]]:
        """claim 后、commit 前验证候选仍可安全 commit。

        Returns:
            ``(block_reason, stale_sor_ids)``——``block_reason == ""`` 表示通过。
            - ``"sensitive_partition"``（P1 第三层最后闸）：候选 ``is_sensitive`` /
              候选 partition 敏感 / 任一源 SOR partition 敏感。发现端两层防御后
              正常管道不产敏感候选——这层防的是存量数据/手工插入/发现端演化漏网。
              敏感 MERGE commit 会被 ``_safe_sor_content`` 毁内容（P1-2），一律拒绝。
            - ``"stale_sources"``（P2）：任一源 SOR 不存在或已非 ``current``（pending
              期间被更新/删除/被其他已 accept 的候选合并）。用旧提议 commit 会让
              新 current 与合并事实并存，绝不静默执行。

        敏感判定单一事实源 ``SENSITIVE_PARTITIONS``（octoagent.memory.enums，与
        write_service ``_safe_sor_content``/``persist_vault`` 同一判定源）。
        """
        if (
            candidate.is_sensitive
            or candidate.partition in SENSITIVE_PARTITIONS
        ):
            return "sensitive_partition", []

        stale: list[str] = []
        for sid in candidate.source_sor_ids:
            record = await self._memory_service.get_memory(
                sid, layer=MemoryLayer.SOR
            )
            if record is None:
                stale.append(sid)
                continue
            sor: SorRecord = record  # layer=SOR 恒返回 SorRecord | None
            if sor.partition in SENSITIVE_PARTITIONS:
                # 源级敏感防御：候选字段可能被数据侧改写，源 partition 是 SOR 层事实
                return "sensitive_partition", [sid]
            if sor.status is not SorStatus.CURRENT:
                stale.append(sid)
        if stale:
    ...
    async def _conflict_candidate(
        ...
        *,
        reason: str,
        stale_sor_ids: list[str],
    ) -> ApprovalResult:
        """验证失败 → APPLYING→CONFLICT（终态，CAS）+ emit CONFLICTED + 409 语义结果。

        不回滚 PENDING：候选内容已失效（源过期/敏感），重审也不能 commit——终态
        避免用户反复 accept 反复失败。CAS expected=APPLYING（本调用刚 claim 的行）。
        **不碰 SOR、不 commit proposal**（C4 安全）。
        """
        marked = await self._consolidation_store.mark_candidate_status(
        ...
        if not marked:
            logger.warning(
                "consolidation_mark_conflict_cas_failed",
                candidate_id=candidate.candidate_id,
            )
        await self._emit_conflicted(
            candidate=candidate, reason=reason, stale_sor_ids=stale_sor_ids
        )
        if reason == "sensitive_partition":
            detail = (
                "候选涉敏感分区（HEALTH/FINANCE）——v0.1 不支持敏感记忆合并"
                "（commit 会丢失敏感内容），候选已标 conflict 失效；"
                "敏感合并将在 v0.2 vault-aware MERGE 支持"
            )
        else:
            detail = (
                "候选的源记忆在待审期间已变更（被更新/删除/被其他合并取代），"
                f"旧提议不再可安全执行，已标 conflict 失效；过期源：{', '.join(stale_sor_ids)}。"
                "如仍需合并，等下次巩固运行基于最新记忆重新提议"
            )
        logger.info(
            "consolidation_accept_conflicted",
            candidate_id=candidate.candidate_id,
            reason=reason,
            stale_count=len(stale_sor_ids),
        )
        return ApprovalResult(
            ok=False,
            status="conflict",
    ...
    async def _emit_conflicted(
        ...
        *,
        ...
        payload = ConsolidationConflictedPayload(
            run_id=candidate.run_id,
            candidate_id=candidate.candidate_id,
            reason=reason,
            stale_sor_ids=stale_sor_ids,
        )
        # actor=SYSTEM：CONFLICTED 是系统检测（源过期/敏感防御）非用户决策——
        # 与 APPROVED/REJECTED（actor=USER）区分，不污染"用户拒绝"审计语义。
        await self._append_event(
    ...
    async def _append_event(
        ...
        *,
        ...
        event = Event(
            ...
            ts=datetime.now(UTC),
            type=event_type,
            # 默认 USER：人审动作（accept→APPROVED / reject→REJECTED，C7 用户主动）；
            # 系统检测（CONFLICTED）显式传 SYSTEM。
            actor=actor,
            payload=payload,
            trace_id="",
        )
