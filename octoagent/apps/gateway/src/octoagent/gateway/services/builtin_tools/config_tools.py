...
async def register(broker, deps: ToolDeps) -> None:
    ...
    async def config_inspect(section: str = "") -> str:
        """读取 octoagent.yaml 配置。

        section 可选: providers, model_aliases, memory, channels, runtime，留空返回全部。
        """
    ...
    async def config_add_provider(
        ...
        """添加或更新一个 LLM Provider 到 octoagent.yaml（单一事实源）。"""
        config = _load_config(deps.project_root)
        if config is None:
        ...
        return ConfigAddProviderResult(
            ...
            hint=(
                "配置已保存；当前进程需要时请重启 Gateway，也可使用 Web 设置页或 CLI `octo setup`。"
            ),
        )
    ...
    async def config_set_model_alias(
        ...
        """设置模型别名（如 main, cheap）到具体 Provider + 模型。"""
        config = _load_config(deps.project_root)
        if config is None:
        ...
        if provider.strip() not in provider_ids:
            return ConfigSetModelAliasResult(
                ...
                model=model.strip(),
                reason=(
                    f"UNKNOWN_PROVIDER: Provider '{provider}' 不存在，可用: {sorted(provider_ids)}"
                ),
            )
    ...
    async def setup_quick_connect(draft_json: str) -> SetupQuickConnectResult:
        ...
        用户提供 API Key 后，通过此工具完成配置与凭证持久化。
        这是安全的凭证注入通道——凭证写入 .env（不进版本管理），
        ...
        if isinstance(result.data, dict):
            ...
            provider_id = str(result.data.get("provider_id", ""))
            alias = str(result.data.get("alias", ""))
