...
class EventType(StrEnum):
    ...
    #   CONFLICTED：accept 时系统检测候选已失效（源 SOR 非 current / 敏感分区防御）→
    #     候选转 conflict 终态，**不 commit**。区别 REJECTED：这是系统检测非用户决策
    #     （actor=SYSTEM），不污染"用户拒绝"审计语义。
    ...
    MEMORY_CONSOLIDATION_PROPOSED = "MEMORY_CONSOLIDATION_PROPOSED"
    ...
    TOOL_RESULT_EVICTED = "TOOL_RESULT_EVICTED"             # F126 项2: 旧 tool 结果 tail eviction 折叠为确定性占位（指向 artifact）
