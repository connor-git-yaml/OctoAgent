...
- **H2 PROTECTED 区段字节级保留**：`<!-- 🔒 PROTECTED -->...<!-- /🔒 PROTECTED -->` 区段走**占位符方案**——发现端先提取区段、替换成 `<<<PROTECTED_n>>>` 占位符再送 LLM（LLM 根本看不到也改不了受保护内容）；LLM 输出中每个占位符必须 **exactly-once**（缺失/重复 → 丢弃 + SKIPPED(protected_violation)）；确定性替换回原文后再做终局字节级包含断言（构造性保证 + belt-and-braces）。占位符方案强于"输出比对"方案：篡改在构造上不可能，且省 prompt 预算。
...
- **H6 USER.md 机器可读字段 parity（F111 特有，收窄期新增）**：USER.md 在 compact 范围内（拍板①），但它承载 F102/F115/F127 的机器可读配置字段（`user_timezone` / `daily_summary_time` / `routine_active` / `summary_channels` / `consolidation_*`）——LLM 精简若把这些行"合并掉"，会静默改变 cron/通知/巩固行为。护栏 = **复用生产 extractors**（`daily_routine_config` / `consolidation_config` 的 `extract_*_from_user_md`，单一事实源）对 original 与 compacted 各跑一遍，任一提取值不一致 → 丢弃 + SKIPPED(config_drift)。这是契约级确定性对账（复用生产解析函数），不是关键词判重规则，C9 合规；未来新增 config 字段未覆盖时的语义 = 少保护不误伤（fail-open 到 H4 人审 diff）。
...
- **FR-11 输入/输出预算（C6）**：输入超 `COMPACT_INPUT_CHAR_BUDGET`（12000）→ SKIPPED(too_large)（**不截断**，§0.2 归档偏离）；输出 `max_tokens=COMPACT_OUTPUT_TOKEN_BUDGET`（8192，须容纳整文件 ≤4000 字符 + rationale）。
...
- 解析：定位 `===COMPACTED===` 起点 → 截到 `===RATIONALE===`（无 RATIONALE 则到文末，rationale 置默认文案）→ strip 外层 code fence（若 LLM 包了 ```）→ 占位符校验 → 插回。
