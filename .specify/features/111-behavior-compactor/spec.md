...
| 幂等账本 | 输出 content_hash 阻断 {PENDING,APPLYING,APPLIED} | **输入 (file_id, source_hash) 阻断 {PENDING,APPLYING}**——整文件重写下"同源已有待审提议"比"同输出"更对：LLM 输出非确定，同源重跑会产不同输出文本，输出 hash 挡不住重复提议堆积；APPLIED 不需阻断（apply 后文件 hash 即变，同 source_hash 天然不再复现；经 F107 恢复回退后允许重新提议恰是正确语义）。**REJECTED 分路径**（Opus 自审精化，覆盖本行早期"不阻断同 F127"表述）：cron 尊重同源 REJECTED 跳过（输入级幂等下文件不变=完全重复输入，nightly 会为同一被拒源反复提议+通知；文件一编辑 hash 变即自然重提）；手动触发不阻断（用户主动=显式重新决定，保留可重试语义）。输出 content_hash 字段保留作审计 |
