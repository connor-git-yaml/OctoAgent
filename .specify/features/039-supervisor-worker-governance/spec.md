...
3. **A2A-Lite 合同存在，但 live dispatch 没有在运行链中显式归一化**  
   Feature 018 已定义 `TaskMessage` / `DispatchEnvelope` 映射，但 live orchestration 仍主要直接在 `DispatchEnvelope` 上运转，A2A 更像适配器而不是实际的内部传递语义。

因此，039 要解决的不是“再加一种 worker 类型”，而是：

> 把主 Agent 收口成 supervisor，把 worker 规划和权限赋予变成可审查、可批准的能力，并让 live dispatch 真正经过内部 A2A 归一化，从而让主 Agent / Work / Worker(Subagent/Graph) 三层结构在系统里成立。
...
- live dispatch 会在 orchestrator 内部经由 `build_task_message()` / `dispatch_envelope_from_task_message()` 做一次 A2A 归一化，并保留 runtime context / work lineage
...
- `OrchestratorService`
  - live dispatch 走内部 A2A roundtrip
...
- **FR-007**: orchestrator live dispatch MUST 在运行链中完成一次 `DispatchEnvelope -> A2A task message -> DispatchEnvelope` 归一化。
- **FR-008**: A2A 归一化 MUST 保留 `runtime_context` 与 `work_id/session_id/project_id/workspace_id` lineage，不得因 roundtrip 丢失。
- **FR-009**: Feature 039 MUST 提供回归测试，覆盖 tool catalog、worker review/apply、A2A roundtrip 三条主链。
...
- **SC-004**: orchestrator 的 live dispatch 测试能证明 A2A roundtrip 与 runtime context 恢复成立。
