...
## F099 推迟项（F101 主接收，Codex Final F3 归档 + Re-Re-Review N-H1 PARTIAL）
...
| `ApprovalGate` 生产接入（F2 已修 DI，sse_push_fn=None 待绑定）| MEDIUM | F101 | octo_harness 已创建 ApprovalGate 并 bind，但 sse_push_fn=None（SSE 推送卡片无效）；F101 负责晚期绑定 sse_hub |
| 完整三条事件序列 e2e 验证（[E2E_DEFERRED] / AC-E1）| MEDIUM | F101 or 独立测试任务 | 需要 TaskRunner + ExecutionConsole 联合集成 |
| **is_caller_worker 持久化非 attach_input resume 路径（Codex Re-Re-Review N-H1 PARTIAL）**| MEDIUM (defense-in-depth) | F101 / F107 | 2f867e6 仅补 attach_input resume 路径；manual resume / startup orphan recovery / deferred dispatch 三条路径靠 WorkerRuntime hardcoded fallback `_is_caller_worker=True` 兜底。**功能正确**（fallback 在 worker→worker 场景与持久化等价；main→worker 不走 WorkerRuntime → 默认 False 正确），但 defense-in-depth 不完整 |
| F5 RUNNING guard 返回空字符串（Codex re-review PARTIAL）| LOW | F101 | guard 失败返回 `""`，LLM 可能不识别为 error，存在 retry 风险；建议返 `"task_not_running"` |
| `_emit_is_caller_worker_signal` broad-catch 吞异常（Codex re-re-review M-1）| LOW | F101 顺手清 | 与 F4 `_emit_ask_back_audit` 同模式，可接受 |
...
v1.2 — F099 post-re-re-review，0 HIGH 残留；F101 接收 7 项归档（F3 HIGH state machine + F5 PARTIAL + ApprovalGate sse + AC-E1 e2e + N-H1 PARTIAL 其余 resume 路径 + M-1 broad-catch + N-L1 sse_push_fn）
