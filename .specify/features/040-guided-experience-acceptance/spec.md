...
parallel_dependency: "Feature 033/036 均已闭环；040 负责把这些 canonical contract 收口成最终 acceptance gate。"
---
...
**Created**: 2026-03-10  
**Updated**: 2026-03-11
**Status**: Passed（040 与 M4 acceptance gates 已闭合）
...
- backend `setup.apply`
- `ChatWorkbench` 显式消费 `context_continuity`，在运行态 degraded 时显示状态与提示
- `MemoryCenter` 串起 `memory -> operator -> export/recovery`，并优先复用 canonical control-plane action/resource
...
- 重做 033 的 context continuity 领域逻辑
- 重做 036 的 `skills.selection.save`
- 重做 CLI `octo init / octo onboard` 与 Web 设置的 canonical setup 汇流
...
- **FR-008**: `context_continuity` 一旦处于 degraded，040 MUST 用显式状态表达当前上下文连续性缺口，而不是静默省略。
