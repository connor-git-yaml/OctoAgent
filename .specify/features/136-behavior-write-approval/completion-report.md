...
- **escalate_permission 同款 allow-always 404 隐患（pre-existing，未修）**：escalate 也 register
  ApprovalManager 不传 allow_always_eligible=False → 用户对 escalate 选"总是批准"同样会短路
  404 超时。F136 只修 behavior.write_file（spec 范围），escalate 语义上 allow-always 可辩护
  （"这类权限申请以后都批"），但 404 隐患真实存在——**建议后续给 escalate 也传
  allow_always_eligible=False 或实现 gate 侧的 allow-always 语义**（独立小 fix）。
