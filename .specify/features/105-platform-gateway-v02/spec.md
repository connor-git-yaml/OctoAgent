...
- **doctor 对 Slack/Discord 的诊断检查**（OPUS-M2 显式排除）：doctor 现仅显式诊断 telegram（doctor.py L93/L308），新 config 字段被静默忽略不报错；与 slash command 注册同属"运维一次性"域，v0.2 不扩 doctor——handoff 运维指引写明"Slack/Discord 配置校验靠 webhook 探测（验签失败 401 / blocked 403 即配置问题信号）"。
...
| D2 | telegram route 迁移方式 | **routes/telegram.py 模块保留**（路由函数/handler 原样；仅 APIRouter 构造补 `tags=["telegram"]`——OpenAPI 分组非行为面，补偿 main.py 挂载行携带的 tags，OPUS-L2 措辞校正），TelegramChannelAdapter.inbound_router() 返回该 router；main.py 撤 L345 挂载行 | 把路由函数搬进 adapter 文件会删 routes/telegram.py → test_telegram_route.py import 断裂 = 红旗；保留模块 + 迁挂载点 = 同 router 对象同 path 同 handler，等价论证最短 |
...
| D5 | Slack/Discord 授权模型 | **静态 allowlist，默认 deny-all，两平台统一规则**（CODEX-M1 修订）：DM 要求 sender ∈ allow_users；**非 DM（频道/guild）消息要求 channel ∈ allowed_channels 且 sender ∈ allow_users——allowed_channels 为空 = 非 DM 一律拒绝**（不存在"空=放开"语义）；Slack 增可选 `team_id`（设置即校验 event 的 team_id，防 app 被装进非预期 workspace——signing secret 只证"来自 Slack"不证"来自我的 workspace"）；Discord 不加 allowed_guilds（channel id 全局唯一，allowed_channels 已收边界，docstring 说明）；不实现 pairing | telegram pairing 依赖 operator inbox + 状态机（平台特定机器）；单用户系统（Blueprint §0）静态 allowlist 是诚实最小面；open 策略否决（公网 webhook 默认放行违反 Least Privilege）；"allowed_channels 空时仅查 allow_users"否决（CODEX-M1：bot 被误装进他人 server 时扩大可达面） |
...
| D9 | 新平台通知目标解析 | SlackNotificationChannel / DiscordNotificationChannel **每次 notify 惰性解析**，但 eligibility 过滤先行（CODEX-H2 修订）：候选 = **DM 类 runtime binding ∪ CONFIGURED binding**，再过 `resolve_outbound_route(候选)` → binding.conversation_id；无命中返回 False 降级。**多人频道的 runtime binding 永不作为通用通知目标**（除非该频道被显式配置为 default_notify_channel）。ingest 写 binding 时在 metadata 记 `conversation_type`（slack: channel_type 值 im/mpim/channel/group；discord: dm/guild），DM 判定 = slack "im" / discord "dm" | 与 telegram 不对称（telegram 用 first_approved_user——pairing 存储是其用户授权 SoT；新平台的等价"私密授权面"= 与 bot 的 DM + 显式配置）；**频道 last-route 直接当通知目标否决（CODEX-H2）**：owner 在多人频道发过一次任务 → daily summary / 任务状态全发进频道 = 任务标题/摘要泄露给频道成员；bootstrap 冻结否决（重蹈 L1） |
...
| D13 | CONFIGURED 写入面（handoff §2.3） | store 单一入口 `upsert_configured_binding(platform, conversation_id, *, scope_id="", project_id="", metadata=None, agent_profile_id="")`：**agent_profile_id 非空 → raise ValueError**（H1 单点校验；非空写入须等"显式用户拍板"配置面，本版无此面）；消费者 = harness bootstrap 读 `channels.{platform}.default_notify_channel` 非空时 upsert（try/except 降级）。**棘轮升级保留但活跃信号分离**（CODEX-H3 修订，见 D17b）：configured 升级不再使该会话丢失 runtime 活跃排序资格 | 入口不带参数否决（H1 校验点就该物理存在且可测，照 handoff "在该入口做 H1 校验"原文）；无消费者的裸 store 方法否决（死代码红线）——default_notify_channel 给出真实产品价值：配置即可收通知 |
| D17a | 平台重试幂等恢复（CODEX-H1） | slack/discord ingest 的 **duplicate 路径加状态守卫补 enqueue**：create_task 返回 created=False 时，读 task——`status == CREATED`（落盘后从未入队/启动）→ 仍调 `task_runner.enqueue`（实测幂等：create_job 是 INSERT OR IGNORE，job 已存在返回 False 短路；_start_job 走 CREATED→RUNNING CAS）；其他状态跳过 | 仅 created 时 enqueue 否决（CODEX-H1：首投 create_task 成功后 enqueue 前崩溃 → 平台重试判 duplicate → 任务永久卡死，浪费平台 retry 这唯一恢复窗口）；**无脑 duplicate 全 enqueue 否决（实测 create_job 对终态 job 有"重入队"语义——晚到重试会把 SUCCEEDED job 置回 QUEUED，状态守卫必须有）**；telegram 同型缺口是 baseline 既有行为，零变更红线不动，归档 handoff |
| D17b | binding 活跃信号与 kind 解耦（CODEX-H3） | conversation_bindings 新增 nullable 列 **`last_runtime_active_at`**：runtime upsert 恒写 now（insert + conflict-update 均写）；configured upsert **不触碰**（升级已有行时保留原值，新建时 NULL）。resolver v2：tier 2 从"RUNTIME kind 中 last_active 最新"改为"**last_runtime_active_at 非 NULL 的 binding 中按该列取最新**（不分 kind）"；tier 1 explicit / tier 3 唯一 CONFIGURED 不变。既有表加列走 sqlite_init 既有 ensure-column 范式（pragma table_info + ALTER ADD COLUMN，L1061-1105 先例） | 维持 v0.1 "kind==RUNTIME 才进 tier 2" 否决（CODEX-H3：棘轮把活跃会话升级 CONFIGURED 后踢出 tier 2，平台残留的旧 runtime 行反而赢得路由 → 通知发进陈旧会话，US-4 AC-3 承诺被自家 FR 打破）；放弃棘轮否决（配置意图需要 kind 表达）；resolver 行为对 runtime-only / configured-only 集合不变（v0.1 单测影响 = 0 或仅构造调整，plan 验证） |
...
7. **Given** owner 给 bot 发过 DM（存在 conversation_type=im 的 runtime binding），**When** NotificationService 推送（channels 含 "slack"），**Then** 通知送达该 DM；**Given 仅存在多人频道的 runtime binding（无 DM、无 configured），Then 返回 False 降级不投递**（CODEX-H2：频道发言不构成通用通知同意）。
8. **Given** 首次 webhook 在 create_task 成功后、enqueue 生效前失败（task 落盘 status=CREATED 无 job），**When** Slack 以同 event_id 重投，**Then** ingest 判 duplicate 且补 enqueue——任务被恢复执行（D17a）；**Given** task 已 SUCCEEDED 后晚到重投，**Then** 不重入队不重执行。
...
3. **Given** DM 类 runtime binding 后续出现，**When** 通知推送，**Then** runtime 活跃证据（last_runtime_active_at）最新者优先（tier 2 压 tier 3，"用户最后私聊的地方"）；**该优先级不被 CONFIGURED 升级破坏**——同一会话被配置为 default_notify 后，其活跃证据仍参与 tier 2 排序（D17b）。
...
- **FR-B1 config**: `SlackChannelConfig`（config_schema.py）：enabled(default False) / signing_secret_env(default "SLACK_SIGNING_SECRET", env 名 pattern 校验) / bot_token_env(default "SLACK_BOT_TOKEN") / allow_users(list[str], int→str 归一) / allowed_channels(list[str]) / **team_id(str ""，CODEX-M1：设置即校验 event 顶层 team_id，不符 → unauthorized)** / default_notify_channel(str "")；ChannelsConfig 加 `slack` 字段（default_factory，旧 yaml 兼容）。
- **FR-B2 client**: `services/slack_client.py` `SlackApiClient`：httpx 直连（TelegramBotClient 先例，无 SDK），`post_message(channel, text, thread_ts=None)` 调 chat.postMessage（Bearer bot token，token 经 env 解析，缺失时方法降级返回 None + warning）；**显式传 `mrkdwn: false`**（OPUS-L3：Slack API 默认 mrkdwn=true 会把任务结果中的 `*`/`_`/反引号误渲染——与 meta.markdown_capable=False 的纯文本声明保持一致）。
- **FR-B3 service**: `services/slack.py` `SlackGatewayService`：
  ...
  - `_ingest_event`：bot_id 非空/subtype 非空/type 非 message → ignored；授权（D5 修订版：team_id 配置时先校验 event 顶层 team_id；DM[channel_type=="im"] 要求 sender ∈ allow_users；**非 DM 要求 channel ∈ allowed_channels 且 sender ∈ allow_users，allowed_channels 空 = 非 DM 一律拒**）→ 不通过 unauthorized；通过 → NormalizedMessage（channel="slack"，scope_id=`chat:slack:{channel_id}`，thread_id=`slack:{channel_id}` 或 `slack:{channel_id}:thread:{thread_ts}`，sender_id=slack user id，metadata 含 slack_event_id/slack_channel_id/slack_user_id/slack_ts[/slack_thread_ts/slack_channel_type]，idempotency_key=`slack:{event_id}`，event_id 缺失 fallback `slack:{channel}:{ts}`）→ create_task → **enqueue 按 D17a（created → enqueue；duplicate 且 task.status==CREATED → 补 enqueue 幂等恢复）** → runtime binding upsert（conversation_id=channel_id，**metadata 含 conversation_type=channel_type 原值（D9 eligibility 依据）**，try/except WARNING 降级）；
...
- **FR-C3 service**: `services/discord.py` `DiscordGatewayService`：
  ...
  - 授权（D5 修订版）：sender ∉ allow_users → unauthorized_user（route 层仍 200 + type 4 ephemeral 拒绝文案 flags=64——Discord 交互必须有应答，传输层 4xx 会显示"interaction failed"且泄露存在性差异）；**非 DM（含 guild_id 的 interaction）要求 channel ∈ allowed_channels，allowed_channels 空 = guild 频道一律拒**；DM（无 guild_id）仅要求 sender ∈ allow_users；
  - 通过 → NormalizedMessage（channel="discord"，scope_id=`chat:discord:{channel_id}`，thread_id=`discord:{channel_id}`，idempotency_key=`discord:{interaction_id}`，metadata 含 discord_interaction_id/discord_channel_id/discord_user_id[/discord_guild_id]）→ create_task → **enqueue 按 D17a（duplicate 且 task.status==CREATED → 补 enqueue）** → binding upsert（**metadata 含 conversation_type："dm"[无 guild_id] / "guild"**）→ 响应 payload type 4（"已受理 task-xxx"）；
...
- **FR-D1 store**: `upsert_configured_binding(platform, conversation_id, *, scope_id="", project_id="", metadata=None, agent_profile_id="")`（conversation_binding_store.py，**单一 CONFIGURED 写入口**）：agent_profile_id != "" → raise ValueError（H1 单点校验，错误信息指明"非主 Agent 绑定需未来显式用户拍板配置面"）；UNIQUE 四元组 upsert，binding_kind 恒 CONFIGURED；已存在 runtime 行被升级为 configured（CONFIGURED 是用户显式意图，优先于 inbound 痕迹；反向 runtime 不降级 configured 的既有语义不变）；**升级时保留该行 last_runtime_active_at 原值、新建时该列 NULL**（D17b：配置动作不伪造也不抹除活跃证据）。
- **FR-D2 通知渠道**: `SlackNotificationChannel` / `DiscordNotificationChannel`（notification.py，channel_name="slack"/"discord"）：`__init__(*, send_fn, binding_store)`；notify() per-call：**eligibility 过滤（D9）——候选 = list_by_platform(platform) 中 [DM 类 runtime（metadata.conversation_type ∈ DM 集）∪ kind==CONFIGURED]** → `resolve_outbound_route(候选)` → None 返回 False（debug log）→ 命中则 send_fn(conversation_id, text)；send_approval_request() 恒 False（§2.2 排除交互审批）；binding_store 异常 → False 降级。
...
- **FR-D4 resolver 文档化**（D11/OPUS2-L2）: resolve_outbound_route docstring 补 list-order 契约说明；explicit 签名不扩参。
- **FR-D5 L3 评估落档**（D12）: 评估结论写本 spec + handoff（chat 级够用 + 失效条件）。
- **FR-D6 活跃信号列 + resolver v2**（D17b/CODEX-H3）: ①conversation_bindings 加 nullable 列 `last_runtime_active_at`（sqlite_init ensure-column 范式：pragma table_info 检测 + ALTER TABLE ADD COLUMN，幂等，存量实例兼容）；②`upsert_runtime_binding` insert 与 conflict-update 路径恒写该列 = now；③resolver tier 2 改为"last_runtime_active_at 非 NULL 的 binding 按该列最新"（不分 kind）；tier 1/tier 3 不变；④ConversationBinding 模型加 `last_runtime_active_at: datetime | None = None`；⑤v0.1 单测影响评估：runtime-only / configured-only 集合行为不变（runtime 行恒有该列值 → tier 2 等价；configured-only 无 runtime 证据 → tier 3 等价）——仅"混合 kind"新场景行为变化，v0.1 既有断言预期 0 修改（若实测需调整构造则逐条等价论证进 verification-report）。
...
| **H1 管家 mediated** | **最高不变量，应用层写入面构造性收敛**（OPUS-M3 措辞校正——保证层级是应用层非 DB 级）：v0.2 全部两个 store 写入口中，runtime 入口签名物理无 agent_profile_id 参数、configured 入口非空必 raise（D13）；新平台 binding 恒指主 Agent；Slack/Discord inbound 无任何 agent 选择参数。DB 列级无 CHECK 约束是有意为之（未来显式配置面演进空间）；SC-5 的 grep 验证范围 = 应用层调用点（raw SQL 不在主张内） |
| H2/H3 | 不涉及 |
...
- **R7 CONFIGURED 升级覆盖 runtime 行**（FR-D1）：同一 conversation 先有 runtime 后配 CONFIGURED → kind 升级，反向不降级——单向棘轮语义在测试固化；**活跃信号经 last_runtime_active_at 独立保留（D17b），升级不再影响出站路由排序**。
- **R8 e2e hermetic 凭证清单**（OPUS-M1）：e2e_live/conftest `_CRED_ENV_KEYS_TO_CLEAR` 必须纳入 SLACK_SIGNING_SECRET / SLACK_BOT_TOKEN / DISCORD_BOT_TOKEN（开发者本机真实 token 不得泄漏进 e2e harness）——Phase B/C 各自补齐。
- **R9 telegram 同型 enqueue 缺口**（CODEX-H1 旁支）：telegram ingest 仅 created 时 enqueue，存在与 D17a 同型的"落盘未入队"窗口（baseline 既有，webhook 重投/polling 重读同样判 duplicate 不补队）——零变更红线不在 v0.2 修，归档 handoff 供独立 fix Feature 评估（TaskRunner.startup 的 _dispatch_queued_jobs 仅恢复已建 job 的场景，不覆盖 job 从未建出的场景）。
...
| US-2 AC-7 | apps/gateway/tests/test_f105v02_outbound.py::test_slack_notification_resolves_dm_last_route + test_channel_only_runtime_binding_not_notified |
| US-2 AC-8 | apps/gateway/tests/test_slack_service.py::test_retry_recovers_unenqueued_task + test_late_retry_after_success_no_requeue |
...
## 11. Pre-impl 双评审闭环记录（2026-06-12）

**Codex adversarial review**（GPT-5.4，verdict needs-attention，3 HIGH + 1 MED）+ **Opus 第二评审**（APPROVE-WITH-CHANGES，0 HIGH / 3 MED / 4 LOW，事实核查 16/16 通过，baseline 3931 实跑复现）。

| Finding | Severity | 处理 |
|---------|----------|------|
| CODEX-H1 平台重试把已落盘未入队任务永久卡死 | HIGH | **接受**：D17a duplicate 路径状态守卫补 enqueue（task.status==CREATED 才补；实测 create_job 终态重入队语义证明"无脑全 enqueue"危险）；US-2 AC-8 双测试；telegram 同型 baseline 缺口归档 R9/handoff |
| CODEX-H2 频道 runtime binding 升级为通用通知目标 = 信息泄露 | HIGH | **接受**：D9 修订——通知 eligibility = DM 类 runtime ∪ CONFIGURED；ingest 写 conversation_type metadata；US-2 AC-7 增反向断言（频道-only 不投递） |
| CODEX-H3 CONFIGURED 棘轮把活跃会话踢出 runtime tier | HIGH | **接受**：D17b 新列 last_runtime_active_at（runtime 恒写 / configured 不触碰）+ resolver tier 2 改按活跃证据排序不分 kind；US-4 AC-3 重述；FR-D6 落 v0.1 单测影响评估 |
| CODEX-M1 Discord 空 allowed_channels 放开所有频道 + Slack 缺 workspace 边界 | MED | **接受**：D5 修订统一两平台（allowed_channels 空 = 非 DM 一律拒）+ SlackChannelConfig.team_id 可选校验；Discord 不加 allowed_guilds（channel id 全局唯一，已收边界） |
| OPUS-M1 e2e hermetic 凭证清单缺新平台 env | MED | **接受**：R8 + Phase B/C 补 `_CRED_ENV_KEYS_TO_CLEAR` |
| OPUS-M2 doctor 对新平台零覆盖（静默忽略） | MED | **接受（显式排除）**：§2.2 增排除条目 + handoff 运维指引写"配置校验靠 webhook 探测" |
| OPUS-M3 H1"物理上不存在口子"措辞强于实际保证层级 | MED | **接受**：§7 H1 行收敛为"应用层写入面构造性收敛"+ SC-5 grep 范围显式化（raw SQL 不在主张内；DB CHECK 有意不加） |
| OPUS-L1 test_control_plane_e2e 是挂载循环的正向回归守卫 | LOW | **接受**：plan EQ-A2 补正向证据 |
| OPUS-L2 EQ-A6 tags 与 D2"原样"措辞矛盾 | LOW | **接受**：D2 措辞校正（handler 原样，APIRouter 构造补 tags 非行为面） |
| OPUS-L3 Slack chat.postMessage 默认 mrkdwn=true ≠ 纯文本声明 | LOW | **接受**：FR-B2 显式传 mrkdwn:false（recon §6 检索盲点，Opus 补全） |
| OPUS-L4 plan 中 daily_routine_config 路径笔误 | LOW | **接受**：plan 路径已纠正（services/ 直下无 config/ 子目录） |

**双评审分歧人裁清单**（主 session 裁定 + 理由，最终报告呈用户复核）：
