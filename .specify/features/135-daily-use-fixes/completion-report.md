...
| `410e60cd` | **gap-2 Codex P2 闭环**：PATH schema marker 让已装服务自愈重写 + migration test |
| （本 commit） | docs：completion-report + Codex finding 闭环 + PATH schema marker living-doc |
...
`codex review --base master` 完成，2 finding：1 P1（HIGH）+ 1 P2（MEDIUM）。

### P2（MEDIUM）— PATH 迁移不刷新已装服务 → **已修闭环（commit `410e60cd`）**
Codex 抓到真问题：PATH 值被 `definitions_equivalent` 剔除（易变字段），已部署 F129 用户
（**正是本次用户情形**）升级后 `octo service install` 走 `skipped`，plist 不重写→保留旧 PATH
（无 node）→npx MCP 继续失败，除非手动 `--force`。这让 gap-2 修复对已部署用户**失效**。
**修复**：新增 `SERVICE_PATH_SCHEMA_VERSION`（v1→v2）+ `OCTOAGENT_PATH_SCHEMA` 非 PATH env marker
写进 plist/unit，参与幂等比对→PATH 逻辑变更时 bump 触发已装服务自愈重写。测试
`test_path_schema_bump_triggers_refresh_on_installed_service` 验证 + 同版本 PATH 变化仍 skip 不回归。

### P1（HIGH）— behavior.write_file 的 confirmed=true 自确认绕过 Two-Phase → **升级用户拍板 + spawn 独立 Feature（未在 F135 内改权限模型）**

**Codex 判断有效且我经验复现证实**：首次调用 `behavior.write_file(confirmed=true)`（无任何前置
proposal）会**直接写入** REVIEW_REQUIRED 的 USER.md（`scratchpad/repro_p1.py` 实测 status=written /
文件真被写）。handler 的 `confirmed` 是 LLM 自填的执行参数，服务端不校验"是否真有过 proposal +
用户确认"→模型/prompt injection 一轮自确认即绕过人审。

**为什么不在 F135 内改**（rigor + 遵守约束）：
1. **pre-existing，非 F135 引入**：`behavior_write_file` handler 的 confirmed 逻辑自 F084/F107 起就这样，
   `git diff master -- misc_tools.py` **为空**（我没碰 handler）。F135 前该缝**已可达**——`tool_search`
   两跳后同样能 `confirmed=true` 写。两跳链路**从来不是安全边界**，只是发现层的 UX 税。
2. **F135 确实放大暴露面**（诚实）：Deferred→Core 让它从两跳变一跳，攻击/误用路径也同步变短。
   但安全语义上 F135 是**中性**的（真正缺失的控制=服务端 proposal 绑定，在 F135 前后都缺）。
3. **属权限模型变更 → 任务约束明令不得擅自改**："如果发现是设计如此需产品决策，停下标注让用户拍板，
   别自作主张改权限模型"。且 M6 `CLAUDE.local.md` 早已把此项列为"待核查安全项 → 确认则立独立 fix
   Feature"——Codex 独立复现确认了它，按既定策略应立独立 Feature 而非塞进 F135。

**处置**：①completion-report + 本节透明记录（不粉饰"F135 放大了暴露面"）；②**已 spawn 独立安全
Feature**（confirmed 步骤接 ApprovalGate / server-side proposal token，消除 LLM 自证确认）；
③**升级用户拍板**（见回报 §e）：是否接受 F135 先合入（UX 修复 + 安全中性）+ 独立 Feature 补 ApprovalGate，
还是要求 P1 与 F135 同期修（会把 F135 从 fix 扩成 L 规模权限模型改造）。

**0 HIGH 残留判定**：P1 作为 pre-existing 缺陷**升级用户决策 + 独立 Feature**，不阻断 F135 合入
（F135 本身 0 新引入 HIGH——它没写/改任何绕过治理的代码，只改发现层可见性）。P2 已在 F135 内闭环。
