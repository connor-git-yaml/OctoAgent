...
当前 `MemoryCenter` 已接入 canonical memory resource，但产品表达还有三个直接影响可用性的缺口：
...
3. **启用路径和配置路径没有最短闭环**  
   用户不知道本地 Memory 已经可以直接工作，也不知道什么时候才需要切到 MemU bridge；即使需要配置，也不清楚最少只需要去 `Settings > Memory` 补哪几项字段。
...
作为想启用或排查 Memory 的用户，我希望页面直接告诉我“基础使用不需要额外服务”“增强检索只需要去 Settings 补 Bridge 地址和 API Key 环境变量”，而不必理解 memu 独立部署、索引细节或 backend 实现。

**Why this priority**: 这正是用户请求里的核心诉求。产品应该暴露最少配置面，而不是把部署细节推给普通用户。

**Independent Test**: 在 `local_only`、`memu 但 bridge_url 缺失`、`memu 但 bridge_api_key_env 缺失` 三种配置下打开页面，验证页面都能给出精简的清单式指引，并带到 Settings 的 Memory 分区。

**Acceptance Scenarios**:

1. **Given** 当前模式是 `local_only`，**When** 用户查看指引区，**Then** 页面说明基础记忆已经可用，不需要额外 Memory 服务，只需先在 Chat 或导入中产生内容。
2. **Given** 当前模式是 `memu` 且 `bridge_url` 为空，**When** 用户查看指引区，**Then** 页面明确提示去 `Settings > Memory` 补齐 Bridge 地址。
3. **Given** 当前模式是 `memu` 且 `bridge_api_key_env` 为空，**When** 用户查看指引区，**Then** 页面明确提示去 `Settings > Memory` 补齐 API Key 环境变量名。
4. **Given** 当前模式是 `memu` 且配置完整，**When** 用户查看指引区，**Then** 页面说明增强检索已经接通或当前正在降级回退，而不是继续显示“待配置”。
...
- **FR-001**: `MemoryCenter` MUST 先输出用户可理解的状态摘要，明确区分“正在工作 / 已就绪但暂无内容 / 配置未完成 / 当前降级”。
...
- **FR-005**: 当当前模式为 `memu` 且最小配置不完整时，页面 MUST 明确列出缺失项，且只暴露用户需要补的最小字段（如 `Bridge 地址`、`API Key 环境变量名`）。
...
- **MemorySetupGuideItem**: 指向用户下一步的最小动作项，如“去 Settings > Memory 补 Bridge 地址”“先去 Chat 产生第一批上下文”。
...
- **SC-003**: 当需要增强检索配置时，页面只要求用户关注最少的 Memory 配置字段，并能直接跳到 `Settings > Memory`。
