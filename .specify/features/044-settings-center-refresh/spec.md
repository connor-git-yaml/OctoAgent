...
**Updated**: 2026-03-14
**Status**: Draft  
**Input**: 重新设计 `/settings`：修复布局混乱、说明过长和内部话术外露；Provider 改为多实例共存并可增删；模型别名只引用 provider + model；Memory 配置明确支持 `local_only / memu + command / memu + http`；删除 Butler 相关模块与文案。
...
- **FR-007A**: Memory 分区 MUST 显式支持 `local_only`、`memu + command`、`memu + http` 三条路径，并按 transport 条件收敛字段，不得把 HTTP/command 字段同时平铺给用户。
- **FR-007B**: Memory 分区 SHOULD 提供与 Web 当前选择一致的 CLI snippet，至少覆盖 `octo config memory local`、`memu-command`、`memu-http`。
