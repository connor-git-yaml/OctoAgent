...
**Updated**: 2026-03-14
**Status**: Draft  
**Input**: 重新设计 `/settings`：修复布局混乱、说明过长和内部话术外露；Provider 改为多实例共存并可增删；模型别名只引用 provider + model；Memory 配置简化为 `local_only` 单一模式；删除 Butler 相关模块与文案。

> **2026-03-17 更新**：MemU bridge 实现已整体移除。Memory 配置已简化为 `local_only` 单一模式，以下涉及 `memu + command / memu + http` 的段落仅作历史参考。

## Problem Statement

当前 `SettingsCenter` 已经能接 setup-governance 主链，但产品层仍存在五个明显问题：
...
5. **Memory 配置仍缺 transport 级语义收口**
   Memory 不应再被表达成“只有一个远端 bridge 地址要填”的模型。对于同机 MemU，产品应优先表达本地 command；对于远端部署，再表达 HTTP bridge。Web Settings 与 CLI 也必须保持同一套配置语义。
...
- **FR-007A**: ~~Memory 分区 MUST 显式支持 `local_only`、`memu + command`、`memu + http` 三条路径~~（已简化：仅保留 `local_only` 模式，MemU bridge 已移除）。
- **FR-007B**: Memory 分区 SHOULD 提供与 Web 当前选择一致的 CLI snippet，覆盖 `octo config memory show` 和 `octo config memory local`。
