...
- 控制面已新增 `session.new / session.reset` 正式动作；Web Chat 的 `开始新对话` 现在会调用服务端 action，下一次发送也会回写 `session.focus`，不再只是本地清状态；Session Center 也已正式暴露 `开始新对话 / 重置 continuity`
...
- Worker runtime 默认也已切到 planner-capable `hint-first`：`research / dev / general` archetype 默认 `planner_enabled=true`，可以直接走 `RecallPlan`；只有显式 profile override 才会回到 `detailed_prefetch + planner_disabled`
...
- Worker runtime 默认改为 `hint-first` memory runtime，与 Agent Zero / OpenClaw 的 query-led recall 方向对齐；如有明确需要，仍可通过 profile override 强制回到 `detailed_prefetch`
...
- Session replay 现在先保留完整 turn-store 投影，再由 prompt budget 过程按预算逐级裁剪；不再固定写死最近 `6/4` 条 dialogue/tool exchange，也不会因为 replay trim 误触发 delayed recall materialization
- Session Center 已从“只能新建/重置”推进到正式产品面：新增 `session.unfocus`，并提供 `全部 / 运行中 / 队列 / 历史` lane 视图
- 默认 `single_loop_executor` 已从 general Butler 扩到显式 worker lens：当 `requested_worker_type=research/dev/ops` 时，Butler 可直接借对应 worker 视角与工具集进入单循环执行；`ButlerDecision/ButlerLoopPlan` 保留给 compatibility / explicit delegation / legacy preflight 场景
- Behavior workspace CLI 已从只读视图扩展为 `edit / diff / apply` 可操作主路径，Settings 页也同步暴露对应入口
