...
**Created**: 2026-06-27
**Status**: **Locked v0.1**（用户已拍板 5 决策为推荐窄路径，见 §0.3；v0.2 项移入 §2.2 deferred）。**Phase A + B + C + D 已实施并测试通过**（162 F127 测试全 PASS）：A 数据模型地基 + B spawn 编排（Opus 自审 2 真回归已修，§0.1.4）+ **C 发现端**（ConsolidationDiscoveryService 确定性组件，拉窗口→LLM 识别冗余→产 WriteProposal[MERGE] validate-no-commit→写 PENDING 候选→emit PROPOSED；16 review 测试 + 7 wiring 测试）+ **D 破坏性 MERGE 人审 C4/C7 红线**（ConsolidationApprovalService accept→atomic claim→commit MERGE 源 SUPERSEDED / reject 不碰 SOR；REST `/api/consolidation/candidates` GET+accept+reject+bulk_reject；14 approval + 8 route 测试，含 subject_key 碰撞优雅回滚 + C4 红线静态证明）。**Phase C 架构偏离归档**：发现端做成确定性组件（llm_client 注入式可单测）而非 subagent free-loop——理由①FR-B `[@test]` 要求确定性单测；②`tool_profile="minimal"` 当前挂不到任何工具（无 builtin 工具标 minimal，需先打通 per-tool profile override，spec 第一决策点未拍板）；spawn 仍是 H2 对等审计容器。**实施期修真 bug**：events 表 PK(task_id,task_seq) 冲突（多事件挂同一 root_task 硬编码 task_seq=0 必撞）→ 改用 append_event_committed 自动 MAX+1 重试 + 先 commit 状态再 emit 防 seq 重试 rollback 丢状态。**Codex review 环境 OAuth/MCP 不可用（AuthorizationRequired，model refresh timeout）→ Opus 自审为主审闸（抓出 subject_key 碰撞边界 + C4 红线证明），待 push 前用户手动跑 codex。** 0 净回归 vs master（memory 149 + F102 87 + event_seq 3 邻接域全绿）。**Phase E + F 已实施**（F127 全套 179 测试 PASS = 163 + 13 notify + 3 e2e）：**E 巩固完成通知**（FR-E1~E4——仅 proposals>0 发一条 MEDIUM `MEMORY_CONSOLIDATION_PENDING_REVIEW`（引导用户去候选列表审批）；0 提议/FAILED/SKIPPED 全静默（与 finding-E 后台静默边界调和：finding-E 压的是 channel==system 通用任务完成/失败推送，Phase E 是 F102 式专用直调 `notify_task_state_change`，finding-E 三个抑制路径零触碰）；channels 复用 F102 `summary_channels` USER.md 字段（不新增模板字段）；`state_transition_event_id=run_id` sha256 幂等；quiet hours 由 NotificationService 自身处理——**注意语义：MEDIUM quiet 内 discard + NOTIFICATION_DISPATCHED(filtered=true) 审计，非延迟投递**，深夜 03:00 触发若在用户 active_hours 之外推送被丢弃，用户经 Web 候选列表主动发现（FR-C6））+ **F H1/端到端**（三分支全链 e2e：accept 链 TRIGGERED→PROPOSED→COMPLETED→APPROVED 精确 task_seq 序 + 源 SUPERSEDED/新权威 CURRENT raw SQL 断言；reject 链 SOR 零触碰；0 提议链零通知双重否定断言；H1=root task 事件宇宙 ⊆ {MEMORY_CONSOLIDATION_*, NOTIFICATION_DISPATCHED} 无任何对话/模型/A2A 事件——全部真查 event_store 非 mock 自证；**AC-10 SUBAGENT_INTERNAL cleanup 真执行链不在本 e2e 复验**——spawn 契约由 trigger + capability_pack_phase_d 测试守，task_runner 真执行属 F097 既有覆盖）。**Phase E+F codex review 已跑通**（`codex review --base b4477ff1`，本环境此前连挂 3 次本次成功）：唯一 finding [P2] Web-only 用户待审通知服务端死角（session_id=None 不落 /api/notifications + SSE 订不到隐藏系统任务）→ **已修**（session_id="" 全局通知桶，路由契约内）+ 范围外部分归档（SSE 隐藏任务订阅 + 前端通知 UI 未建 + F102 同缺口，归 F101/前端 follow-up）。0 HIGH 残留。**第二轮 codex review 3 finding（2 P1 敏感记忆 + 1 P2 过期候选）已全部闭环**（2026-07-03，4 commits）：[P1-1] 敏感性按推断目标 partition 众数计算，LLM 混组时敏感内容以 is_sensitive=False 走普通明文存储（降级）；[P1-2] 敏感 MERGE 提议 commit 走 `_commit_add`，`_safe_sor_content` 把 content 换成 rationale 且 MERGE 不建 vault——accept 后毁掉敏感记忆；[P2] 候选 pending 期间源 SOR 被更新/删除/被共享源候选合并，直接 commit 旧提议 → 新 current 与合并事实并存。**修法 = v0.1 收窄 + 三层防御 + 新鲜度验证**：①发现端窗口直接排除 `SENSITIVE_PARTITIONS`（根治 P1，敏感合并推 v0.2 vault-aware MERGE，见 §2.2）；②`_propose_group` 敏感判定改 any 语义（任一源敏感即拒绝产候选）；③审批端 accept claim 后 commit 前 `_verify_sources_for_commit`——敏感最后闸（防存量/伪装候选）+ 逐源验证仍 current（P2，任一过期 → 拒绝，绝不静默用旧内容 commit）；④验证失败 → 候选 APPLYING→**CONFLICT 新终态**（CAS，不回滚 PENDING——内容已失效重审无意义）+ emit **MEMORY_CONSOLIDATION_CONFLICTED 新事件**（actor=SYSTEM，与用户决策 REJECTED 区分）+ REST 409（detail 引导等下次巩固重新提议）。残余 TOCTOU（验证与 commit 间 await 点）诚实归档：不动 write_service 事务边界无法消除，兜底=软删可回滚（FR-C5）。测试净 +10（models +2 / review +1 / approval +6 / route +1，两个断言旧 bug 行为的敏感测试重写；F127 全套 170→180 PASS）。**codex 复审迭代 round2-4**（每次修复 commit 后复跑 `codex review`，再抓 2 真 P2 全闭环）：round2 [P2] 幂等账本黑名单式判重（status != REJECTED）会让 CONFLICT 候选吞掉下次巩固基于新 current 源的同内容重新提议——accept 409 引导的恢复主流程断裂 → 改**白名单** `_DUP_BLOCKING_STATUSES={PENDING,APPLYING,APPLIED}`（未来新增终态默认不阻断）+ 完整恢复链 e2e（源更新→409→重新提议放行→新候选 accept 成功）；round3 [P2] `_verify_sources_for_commit` 自身异常（get_memory 临时 SQLite 错误）发生在 claim 后，候选卡死 APPLYING（不在 pending 列表且 claim/CAS 全失败）→ 验证步纳入 try/except 走与 commit 失败相同回滚路径（**语义区分：判定失败=确定性→CONFLICT 终态；自身异常=临时故障→回滚 PENDING 可重试**）+ 注入故障测试（回滚+零 CONFLICTED 误发+恢复后重试成功）；round4 **0 finding 闭环**。最终 F127 全套 182 PASS；gateway 全量 2267 passed vs baseline 2257（+10 新测试，唯一 failed=test_plugin_watcher pre-existing，baseline 单跑同 fail，F106 域与 F127 无关）；memory+core 677 passed；e2e_smoke 8/8；ruff 8 findings 全 pre-existing（enums.py E501×7 F124/F126 注释行 + memory/__init__ I001，baseline 同报，不越范围修）。**G-lite 已完成（2026-07-03，用户拍板 G→G-lite：DeepSeek-V3.2 API key 跑发现端，非强 model；强 model 质量评估归 M7 统一 OctoBench 方案）**：真 LLM 第一次跑发现端 **6/6 PASS**（2 批 ×3 轮独立临时库，每轮 9 项硬断言全过：facts_reviewed=5 / 非 fallback / ≥1 合法 MERGE 提议 / 全 PENDING / source_sor_ids ⊆ 植入冗余组 / 零污染零幻觉 id / C4 源仍 CURRENT / PROPOSED 事件数对齐）；质量观察：①共享源双候选自然复现（佐证 FR-C7 CONFLICT 设计必要性）②LLM 单源伪组被 MIN_GROUP_SOURCE_COUNT 过滤未污染审批队列 ③无 code fence 裸 JSON 被解析兜底吃掉 ④保守拆分带排除理由（prompt"宁缺毋滥"指令生效）；脚本+结果归档 `glite/`（独立验证脚本不进 production 包）。Phase H（文档收口）完成后到 push 闸。
...
- **敏感分区（HEALTH/FINANCE）事实合并**（第二轮 codex review P1-1/P1-2 后收窄）——v0.1 巩固**只处理非敏感事实**：发现端窗口直接排除 `SENSITIVE_PARTITIONS` + `_propose_group` any 语义纵深防御 + 审批端 accept 最后闸（三层）。根因：现有 `write_service` MERGE commit 走 `_commit_add`，敏感 proposal 的 SOR content 被 `_safe_sor_content` 替换成 rationale 且 MERGE 分支不建 vault——accept 会毁掉敏感 merged_content。v0.2 需 **vault-aware MERGE**（MERGE 分支支持 persist_vault 建 vault 存原文 + SOR 存脱敏引用）后才能放开。
...
- **FR-C7（源新鲜度，第二轮 codex P2）**：accept atomic claim 后、commit 前**重新验证全部 source_sor_ids 仍 CURRENT**（候选 pending 期间源可能被更新/删除/被共享源候选合并）；任一不 current → 候选转 **CONFLICT 终态**（CAS APPLYING→CONFLICT，不回滚 PENDING）+ emit `MEMORY_CONSOLIDATION_CONFLICTED`（actor=SYSTEM）+ REST 409，**绝不静默用旧内容 commit**。同一验证步也是敏感分区第三层最后闸（候选/源任一涉 `SENSITIVE_PARTITIONS` → 同样 CONFLICT）。

  - `[@test]` → `octoagent/apps/gateway/tests/test_f127_consolidation_approval.py`（no-self-commit / accept→MERGE commit→SUPERSEDED / reject→不碰 SOR / atomic claim rollback / 软删可回滚 / **源过期→conflict+SOR 零触碰 / 共享源第二候选 conflict / 敏感最后闸**）

### FR-D 数据模型与事件

- **FR-D1**：新增 `MEMORY_CONSOLIDATION_TRIGGERED / COMPLETED / FAILED / SKIPPED`（运行级）+ `MEMORY_CONSOLIDATION_PROPOSED / APPROVED / REJECTED / CONFLICTED`（提议级，CONFLICTED 为第二轮 codex P2 增补——accept 时系统检测候选失效，actor=SYSTEM 区别用户决策）EventType，对齐 `enums.py` 命名惯例。payload 不含敏感原文（合并内容用 hash 或 id 引用，沿用现有 PII 防护惯例）。
...
- **AC-8**（强 model 验证，§9④）：强 model 跑新增"记忆巩固域"OctoBench task，验证巩固真改善 recall 质量（控变量 DeepSeek 照不出，需强 model 单独验证）。**归档（2026-07-03）**：**G-lite PASS（DeepSeek-V3.2，n=6，6/6）**——管道真通 + 提议质量下限（植入明显冗余全部被找到、source_sor_ids ⊆ 冗余组、零污染零幻觉、C4 守住），详见 `glite/result.md`；**剩余部分（recall 改善量化 + 记忆域 OctoBench task + 强 model 评估）→ M7 统一强 model OctoBench 方案**（用户拍板 G→G-lite，避订阅 OAuth ToS 灰色地带）。
...
- **NFR-3（C5 最小权限，第二轮 codex P1 后收窄）**：巩固 subagent `tool_profile` 受限（只需 memory 读 + 提议写，不需 terminal/web）；敏感分区（HEALTH/FINANCE）事实 **v0.1 不进巩固**——发现端窗口排除 + any 语义纵深防御 + 审批端最后闸三层（原"标 is_sensitive 强制人审"语义不成立：敏感 MERGE commit 会被 `_safe_sor_content` 毁内容，见 §2.2 deferred v0.2 vault-aware MERGE）。
