...
| release 半边：`octo attest remote/service --json`（F144）+ attestation-checklist.md（机器可读 YAML）+ e2e_full 真 LLM 套件——**三者从未被编排**，真机验证反复推迟 | release 模式 = 确定性全量（`-m "not e2e_full"`）+ **live-e2e-full lane（skip 即 FAIL）** + `attest service` → `attest remote`（handoff 顺序）+ attestation 签署核对；`SKIP_E2E` 在 release 无效 |
...
- `attest remote` = not_enabled：**默认阻断**（FAIL + 打印 next_steps），显式 `--allow-not-enabled`
  → 降级 WARN 记录放行。堵「忘了部署远程还以为验过」——远程触达是 optional 能力，但 release gate
  的默认必须逼一次显式确认。
- `attest service` = not_enabled：**恒 FAIL 无 flag**——常驻服务是部署形态前提（F129 / handoff §1
  明示「应视为 WARN/FAIL 级」，取 FAIL：release 跑在部署机上，服务未安装是真阻断）。
- `fail`（exit 1）：恒阻断（handoff P2-1：bearer+tailscale 断链钉在 fail，lane 不得当可忽略）。
- 探针顺序 service → remote（handoff §2：service 闪断恢复后再跑 remote，避免撞闪断窗口）。
...
### D4 release「skip 即 FAIL」语义 = lane 级（非逐测试级）

- cc-haha `enforceReleaseLiveLanes`：release 模式下 **live lane 整体被 skip → 强转 FAIL**。
- 机械规则：live-e2e-full lane pass 条件 = `pytest -m e2e_full` exit 0 **且 passed ≥ 1**——
  「passed ≥ 1」抓「凭证缺失 → 全 SKIP → exit 0 假绿」这一最危险静默路径。
- 个别 GATE_P3_DEVIATION SKIP（LLM 未命中目标工具的诚实结构化 SKIP）属 lane 内细节：**记录不阻断**
  ——它们是设计内的 LLM 变异性信号，不是环境缺失；逐测试级 skip=FAIL 会让 release 非确定性阻断。
...
### D5 changed-lines coverage 门机械规则（CI-only）
...
- 覆盖数据：pytest-cov lcov（`relative_files = true` 保证 SF 路径 = `packages/.../src/...` 相对路径，
  与 git 路径头部对齐后可交集）；主跑 + e2e_scripted 步 `--cov-append` 合并。
...
- escape hatch：HEAD commit message 含 `[cov-exempt]` → 门以 exempt 状态 PASS 并在 CI 日志
  显式记录（照 SKIP_E2E 先例：可 bypass、有痕迹）。
...
| backend-full | — | ✅ | — | 半 | 全 testpaths（含 e2e_live；e2e_full 凭证在场即真打，SKIP 记录不阻断） |
| backend-deterministic | — | — | ✅ | — | 全 testpaths `-m "not e2e_full"`（避免与 live lane 双跑真 LLM） |
| frontend-vitest | — | ✅ | ✅ | — | `npx vitest run` |
| l1-playwright | — | 可选 | — | — | `--with-l1` 显式开启（本地需 playwright 浏览器；CI 有独立 job 常跑） |
| live-e2e-full | — | — | ✅ | ✅ | `pytest -m e2e_full`；**exit 0 且 passed ≥ 1**；SKIP 清单记录进报告 |
| attest-service | — | — | ✅ | ✅ | `octo attest service --json`；status=pass 才过；not_enabled → FAIL |
| attest-remote | — | — | ✅ | ✅ | `octo attest remote --json`；not_enabled 默认 FAIL，`--allow-not-enabled` → WARN 放行 |
...
- **AC-3**（release 堵逃生门）：单测证 ①live lane pytest 全 SKIP（exit 0 passed=0）→ lane FAIL；
  ②`SKIP_E2E=1` 环境下 release lane 照跑并打印无效警告；③`--skip live-e2e-full` 在 release → exit 2；
  ④attest remote not_enabled 无 flag → FAIL、有 flag → WARN 放行；⑤attest service not_enabled → FAIL。
