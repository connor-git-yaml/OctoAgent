...
2. **Given** dry-run 已运行，**When** 跑 `octo memory migrate-094 --apply`，**Then** `memory_maintenance_runs` 表新增一条 `idempotency_key="octoagent.memory.migration.094.worker_memory_parity.noop.v1"` / `kind="migration"` / `metadata.no_op=true` 的记录，**SoR 表零修改**。
...
- 必须 idempotent（用 `memory_maintenance_runs` + `idempotency_key="octoagent.memory.migration.094.worker_memory_parity.noop.v1"`）
...
- [ ] **E2** `octo memory migrate-094 --apply` 写入一条 `memory_maintenance_runs` 审计记录（`idempotency_key="octoagent.memory.migration.094.worker_memory_parity.noop.v1"` / `kind="migration"` / `metadata.no_op=true` / `metadata.reason="F063_legacy_no_provenance"`），**SoR 表零修改**；前置依赖块 C C6 已通（idempotency_key 列存在）
...
| 块 E 与 F063 idempotency_key 命名冲突 | 低 | 已验证不同（`octoagent.memory.migration.094.worker_memory_parity.noop.v1` vs migration_063_scope_partition）|
