# F098 A2A Mode + Worker↔Worker — Spec（v0.1 Draft）
...
### 块 B：A2A Receiver target profile 独立加载（H3-B 核心）
...
- 路径 2：按 `worker_capability` 派生 default profile（如 worker_capability="research" → resolve "research" worker profile）
- 路径 3：fallback 到当前行为（兼容性，但需 emit warning log）
...
- audit chain 自动追踪（child Worker 独立 runtime_id / session_id）
...
### 块 H：P2-4 终态统一层（OD-4 选 A）

**当前 baseline**（task_runner.py 多处分散调用 `_close_subagent_session_if_needed`）。

**目标（OD-4 选 A）**：cleanup hook 挪到 task state machine 终态层：

1. `task_service.py` 新增 `_terminal_state_callbacks: list[Callable]` 注册机制
2. `task_runner.py` 注册 `_close_subagent_session_if_needed` 为 callback
...
### 块 B：A2A target profile 独立加载

- **AC-B1**：A2A target Worker 加载自己的 AgentProfile（按 `requested_worker_profile_id`）—— 通过 `_prepare_a2a_dispatch` 单测验证 target_profile.profile_id != source_profile.profile_id
- **AC-B2**：fallback 路径可工作（无 requested_worker_profile_id 时按 worker_capability 派生 default profile）—— 单测验证
- **AC-B3**：A2A receiver context 4 层身份独立（profile / runtime / session / Memory namespace）—— 集成测验证
...
- **AC-H1**：`task_service._write_state_transition` 加 `_terminal_state_callbacks` 注册机制 —— 单测验证 callback 触发
