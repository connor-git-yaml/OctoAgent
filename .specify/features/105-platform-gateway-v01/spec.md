...
- **FR-E3**: 写入点两处，均 try/except WARNING 降级不阻断主链：
  ...
  - **H1 排除规则（CODEX-H4）**：chat.py 当请求显式携带 agent_profile_id（用户选 worker 直聊，含 new_conversation 解析出的 profile）→ **跳过 upsert**（debug 日志）——ConversationBinding v0.1 只登记"主 Agent 默认路由"的会话，direct-worker 会话不得伪装成主 Agent last-route。
