...
**FR-2.4** MUST：所有"写入型工具"（即 `tool_contract` 装饰器声明 `produces_write=True` 的工具）必须返回统一的 `WriteResult` Pydantic 模型，包含字段：`status`（`"written" | "skipped" | "rejected" | "pending"`，**`pending` 专用于异步启动**——如 `mcp.install` 启动 npm/pip 安装 job 后立即返回，但实际安装在后台进行；调用方用子类返回的 `task_id` 通过 `mcp.install_status` 查询进度）、`target`（写入目标的标识符，如文件路径 / DB 表名 / 子任务 ID / 异步 job ID）、`bytes_written`（可空整数）、`preview`（前 200 字符摘要，可空）、`mtime_iso`（可空 ISO 8601）、`reason`（可空，状态非 `written` 时的原因或异步任务说明）。

**写入型工具的判定规则**：工具语义产生持久化 storage / config / state 写入（如文件、SQLite 表、远程服务安装、子任务派发、pipeline run 启动等），与"执行类副作用"区分（如 `browser.open` / `terminal.exec` / `tts.speak` 这类是命令执行 / session 控制 / 音频输出，虽然 `side_effect_level` 也是 REVERSIBLE+，但语义上不是"产生新写入物"，所以 `produces_write=False`，return type 不约束）。

`tool_contract` 装饰器在工具注册期通过 `typing.get_type_hints(handler, include_extras=True)` 解析 handler return annotation（必须 eval string-form forward refs，因为 14/15 个 builtin_tools 启用了 `from __future__ import annotations`），检查 `produces_write=True` 工具的 return type 必须是 `WriteResult` 子类，否则启动时 fail-fast（Constitution C3：Tools are Contracts，schema 与代码签名一致）。
...
**FR-2.7** MUST：现存所有"写入型工具"必须在 Phase 2 内迁移到 `WriteResult` 返回契约，并显式标记 `produces_write=True`。完整清单（≥ 18 个工具，**真实工具名**已与代码库 `rg 'name="..."' builtin_tools/` 核对）：

- **配置写入**（4）：`config.add_provider` / `config.set_model_alias` / `config.sync` / `setup.quick_connect`
- **MCP 安装**（2）：`mcp.install` / `mcp.uninstall`
- **子任务控制**（5）：`subagents.spawn` / `subagents.kill` / `subagents.steer` / `work.merge` / `work.delete`
- **文件写入**（1）：`filesystem.write_text`
- **Memory 写入**（1）：`memory.write`
- **Behavior / Canvas 写入**（2）：`behavior.write_file` / `canvas.write`
- **Pipeline 持久化**（1）：`graph_pipeline`（其 `action="start"` 创建 Task / 保存 Work / commit DB 事务 / 启动后台 run，是真实持久化写入；F15 修复：从执行类清单移入写入型）
- **F084 新增**（2）：`user_profile.update` / `user_profile.observe`

**关键设计：保留下游关联键不被压扁**——每个写工具定义自己的 `WriteResult` 子类，**继承**基础字段（`status` / `target` / `preview` / `bytes_written` / `mtime_iso` / `reason`）的同时**保留**现有结构化字段（`task_id` / `work_id` / `session_id` / `artifact_id` / `memory_id` / `version` / `children[]` 等），后续 steer/kill/inspect/打开 artifact / 追踪 memory commit 的关联键不丢失。例如：
- `SubagentsSpawnResult(WriteResult)` 加 `requested: int` / `created: int` / `children: list[ChildSpawnInfo]`
- `SubagentsKillResult(WriteResult)` 加 `task_id: str` / `work_id: str` / `runtime_cancelled: bool` / `work: WorkSnapshot | None`
...
- `ConfigAddProviderResult(WriteResult)` 加 `provider_name` / `yaml_path` / `credential_added`
- 其他工具按现有 return shape 定义对应子类

**显式排除**（保留原 return type，`produces_write=False` 默认）：`browser.open` / `browser.navigate` / `browser.act`（浏览器 session 控制）/ `terminal.exec`（命令执行）/ `tts.speak`（音频输出）—— 这些是"执行类副作用"，不产生持久化写入物。**注**：`graph_pipeline` 之前被误归到此类，R8 review 发现其 `action="start"` 会写 SQLite Task/Work 表 + commit 事务，已移到写入型清单（F15 修复）。

迁移完成后，注册期 schema 检查全部通过；单元测试 `test_all_produces_write_tools_return_write_result` 扫描注册表自动发现所有 `produces_write=True` 工具（按真实 tool name 断言），断言 return type 是 `WriteResult` 子类（避免清单与扫描结果脱钩）。此 FR 把"USER.md 写入回显"问题升级为通用的"写入回显契约"，避免 case-by-case 修复。
...
- **SC-012** WriteResult 契约覆盖：所有 `produces_write=True` 工具返回类型为 `WriteResult` 子类；注册期通过 `typing.get_type_hints` 解析（兼容 `from __future__ import annotations` 字符串化注解）；单元测试扫描注册表，断言 `produces_write=True` 工具数 ≥ 18（含 `graph_pipeline`）且 return type 100% 合规；任何不符合的写工具，启动时 fail-fast（FR-2.4 / FR-2.7）
