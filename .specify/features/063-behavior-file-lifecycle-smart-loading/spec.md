...
1. **BOOTSTRAP.md 永久注入**：Agent 完成 onboarding 后，BOOTSTRAP.md 仍然每次 session 都注入 system prompt，浪费约 2200 字符 token 预算且内容已无意义。OpenClaw 通过 "LLM 自删除" 机制解决了这个问题。
...
- **US-3**：作为 Subagent，我只收到最小行为子集（AGENTS + TOOLS + IDENTITY），保证上下文精简、任务聚焦。
...
| FR ID | 描述 | 来源 | 验收标准 |
|-------|------|------|---------|
| FR-1 | BOOTSTRAP.md 支持"已完成"状态标记，完成后不再注入 system prompt | US-1 | Agent 完成 onboarding → 标记完成 → 后续 session 不含 BOOTSTRAP.md 内容 |
...
| FR-6 | Subagent 使用 MINIMAL profile | US-3 | Subagent 只收到 AGENTS + TOOLS + IDENTITY |
| FR-7 | 行为文件总大小监控，超过阈值时发出警告事件 | US-4 | 总大小 > 阈值 → Event Store 记录警告 |
| FR-8 | 内置 Behavior Compactor（可选自动或手动触发） | US-4 | 压缩后总大小下降，压缩前自动备份 |
